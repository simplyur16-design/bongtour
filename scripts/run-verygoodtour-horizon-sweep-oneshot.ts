/**
 * verygoodtour registered 전체 — 미래 180일 출발·가격 1회성 순차 sweep.
 *
 * SSOT: `lib/verygoodtour-sweep` `sweepDueVerygoodtourProducts` (HXR → E2E 폴백).
 *
 *   npm run db:verygoodtour-sweep-oneshot
 *   npm run db:verygoodtour-sweep-oneshot -- --dry-run
 *   npm run db:verygoodtour-sweep-oneshot -- --resume
 *   npm run db:verygoodtour-sweep-oneshot -- --from-slug pkg-vg-0001
 *
 * 상태: `ops/verygoodtour-horizon-sweep-oneshot-state.json`
 * 상품 간 대기: `VERYGOOD_ONESHOT_PAUSE_MS` (기본 2500)
 */
...
import { sweepDueVerygoodtourProducts, type VerygoodtourSweepResult } from '@/lib/verygoodtour-sweep'

const STATE_PATH = path.join(process.cwd(), 'ops', 'verygoodtour-horizon-sweep-oneshot-state.json')
...
type OneshotState = {
  ...
  last?: {
    slug: string | null
    originCode: string | null
    result: VerygoodtourSweepResult
  }
  log: Array<{
    at: string
    index: number
...
function interProductPauseMs(): number {
  const raw = Number(process.env.VERYGOOD_ONESHOT_PAUSE_MS ?? '2500')
...
async function main() {
  ...
  for (let i = startIndex; i < targets.length; i += 1) {
    ...
    if (!hasUrl) {
      ...
      console.error(`[oneshot] ${i + 1}/${targets.length} SKIP no url slug=${t.slug ?? t.id}`)
      saveState(state)
      continue
    }
    ...
    const result = await sweepDueVerygoodtourProducts(prisma, {
      productId: t.id,
      limit: 1,
    })
