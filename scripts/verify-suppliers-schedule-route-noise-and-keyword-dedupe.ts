...

{
  ...
  const d3 = out.find((r) => r.day === 3)
  assert.ok(String(d1?.imageKeyword ?? '').match(/Ariyabal|Terelj National Park/i), 'mongolia day1 kw1 landmark')
  assert.ok(d1?.imageKeyword2 == null || String(d1?.imageKeyword2 ?? '').trim() === '', 'mongolia day1 kw2 null (departure)')
  ...
  assert.ok(/Zaisan Memorial/i.test(d3kw1) || /Sukhbaatar Square/i.test(d3kw1), 'mongolia day3 kw1')
