...
정본(SSOT): `scripts/calendar_e2e_scraper_hanatourDEV/` — 실전 `calendar_e2e_scraper_hanatour` 는 동일 로직 복제.
    ...
    async def _finalize_hanatour_e2e_run(
        ...
        *,
        ...
        if own and self._pw is not None:
            ...
            self._page = None
            try:
                await asyncio.wait_for(close_hanatour_browser(pw, br), timeout=12.0)
            except Exception:
                pass
