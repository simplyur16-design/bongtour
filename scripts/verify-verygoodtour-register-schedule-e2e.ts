...
async function main() {
  ...
  const bodyPath = path.join(process.cwd(), 'tmp_verygoodtour_body.txt')
  if (!fs.existsSync(bodyPath)) {
    console.error('Missing tmp_verygoodtour_body.txt — add verygoodtour paste sample at repo root first.')
    process.exit(1)
  }

  const text = fs.readFileSync(bodyPath, 'utf8')
  const originSource = 'verygoodtour'

  console.log('--- Preview parse (forPreview: true), handler parity (no skipDetail on first call) ---')
  const preview = await parseForRegisterVerygoodtour(text, originSource, {
    forPreview: true,
  })
  ...
  if ((preview.schedule?.length ?? 0) === 0 && text.trim()) {
    console.log('--- Recovery full parse (handler parity) ---')
    const full = await parseForRegisterVerygoodtour(text, originSource, {
      forPreview: false,
  ...
  console.log('\nverygoodConfirmHasScheduleExpressionLayer:', verygoodConfirmHasScheduleExpressionLayer(aug, drafts))
  ...
  if (!verygoodConfirmHasScheduleExpressionLayer(aug, drafts)) {
    console.error('FAIL: verygoodConfirmHasScheduleExpressionLayer false')
    mismatch++
  }
