...
async function main() {
  const r1 = await fetch(`${baseUrl}/api/travel/parse-and-register-modetour`, {
    method: 'POST',
  ...
  const r2 = await fetch(`${baseUrl}/api/travel/parse-and-register-modetour`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', Cookie: cookie },
  ...
  const brand = await prisma.brand.findUnique({ where: { brandKey: 'modetour' } })
  if (!brand) throw new Error('modetour brand missing')
