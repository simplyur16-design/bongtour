...
import { oauthConfigurationErrorHintEn } from "@/lib/auth/oauth-configuration-error-copy";
...
type Props = { locale: SimplyurLocale; siteOrigin: string };

const STATIC_ERROR_HINTS: Record<string, string> = {
...
export function SimplyurAuthErrorClient({ locale, siteOrigin }: Props) {
  const tr = useSimplyurT();
  const params = useSearchParams();
  const code = params?.get("error") ?? "Default";
  const hint =
    code === "Configuration"
      ? oauthConfigurationErrorHintEn(siteOrigin)
      : (STATIC_ERROR_HINTS[code] ?? tr("auth.errorGeneric"));
