    ...
    <div className="min-h-screen bg-white text-slate-900">
      ...
      <main className="mx-auto max-w-3xl px-4 pb-16 pt-8 lg:max-w-4xl lg:px-6 lg:pb-20 lg:pt-10">
        <section className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm lg:p-6">
          <h2 className="text-lg font-bold text-slate-900 lg:text-xl">소개</h2>
          <p className="mt-3 text-sm leading-relaxed text-slate-900 lg:text-base">{intro}</p>
        </section>

        <section className="mt-6 rounded-xl border border-slate-200 bg-white p-5 shadow-sm lg:mt-8 lg:p-6">
          <h2 className="text-lg font-bold text-slate-900 lg:text-xl">{countriesHeading}</h2>
          <p className="mt-3 text-sm leading-relaxed text-slate-900 lg:text-base">{countriesText}</p>
        </section>

        <div
          className="mt-6 rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm leading-relaxed !text-amber-900 lg:mt-8 lg:p-5 lg:text-base"
          role="note"
        >
          <h2 className="font-semibold !text-amber-900">주의사항</h2>
          <ul className="mt-3 list-inside list-disc space-y-2 marker:text-amber-700">
            {noticeItems.map((item) => (
              <li key={item} className="!text-amber-900">
                {item}
              </li>
            ))}
          </ul>
        ...
        >
          <h2 id="esim-benefit-cs-heading" className="text-base font-semibold text-slate-900 lg:text-lg">
            문제가 있으신가요?
          </h2>
