...
| `SOLAPI_KAKAO_TPL_COUPON_*` | 비즈센터 승인 템플릿 ID 6종 (`.env.example` 참고) |
...
2. 아래 6개 템플릿을 비즈센터에 등록·승인 (영업일 1~3일 소요 가능).
...
| H-1 생일 cron | `notifyCouponBirthday(user, userCoupon)` |
...
`SOLAPI_KAKAO_DRY_RUN=true`일 때 콘솔에 `[kakao-dry-run]` JSON이 6줄 출력되면 정상이다.
