...
## 언제 manifest에 넣나 (필수 — **항상**)

**모든 버그·교정·회귀 수정**에 **같은 PR/커밋**에서 manifest 항목을 추가한다.  
“나중에”, “이번만”, “작은 수정” **없음**.

- 등록·가격·제목·geo·imageKeyword·originCode 등 **데이터 오염**류
- 운영자가 같은 교정을 **두 번 이상** 요청한 버그 (당연히 포함)
- “3일 전에 고쳤는데 또” 류 회귀

**의도적 스펙 변경**만 예외 — 그때도 manifest·가드·테스트를 **같은 PR에서** 새 스펙에 맞게 갱신한다.
