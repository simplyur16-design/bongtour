...
### 2.6 `GET /api/v2/products` — (구 Postman 컬렉션 항목 / **Partner API v2 공식 문서에는 부재**)

| 항목 | 내용 |
|------|------|
| 상태 | **USIMSA Partner API v2 공식 사양(2026-05 기준)에는 이 경로가 없다.** 과거 Postman에만 등장했을 수 있음. |
| 봉투어 | ~~`lib/usimsa/products.ts`~~ **삭제됨** — 가격·카탈로그 SSOT는 USIMSA 엑셀 import (`run-excel-import` 등). |
...
- **공식 v2 REST에는 카탈로그 엔드포인트 없음** — 옵션·가격은 USIMSA 엑셀 등 별도 채널. (구 Postman 예시에 `price` 등이 있었을 수 있음.)
...
- `lib/usimsa/signature.ts`(StringToSign + HMAC-SHA256 + Base64), `lib/usimsa/client.ts`(`usimsaRequest`), `lib/usimsa/config.ts`(dev/prod 액세스 키 분리 + 레거시 단일 키 호환).
- **Partner API v2 공식 4종:** `POST /v2/order`, `GET /v2/order/{orderId}`, `GET /v2/topup/{topupId}`, `GET /v2/topup/{topupId}/usage/daily`. 그 외(Postman의 cancel·extend·products 등)는 스펙 외 — 필요 시 USIMSA에 별도 확인.

### 웹훅 수신 (USIMSA → 봉투어)

- `app/api/usimsa/webhook/route.ts` 및 `app/api/bongsim/webhooks/usimsa/route.ts`: IP 화이트리스트 + `handleUsimsaWebhook` 등(**운영 구현은 봉심 경로가 주력**).
- 레거시 서술: ~~JSON 파싱·로그만~~ → 현재는 topup 반영·배달 플로우까지 연결됨. 서명 검증은 여전히 문서 미확정 시 보완 검토.
