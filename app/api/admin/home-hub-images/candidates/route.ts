...
import {
  deleteHomeHubCandidate,
  isValidCardKey,
  isValidSeason,
  listHomeHubCandidates,
} from '@/lib/home-hub-candidates'
...
export async function GET(request: Request) {
  ...
  const rawCard = searchParams.get('cardKey')?.trim()
  const rawSeason = searchParams.get('season')?.trim()

  if (rawCard && !isValidCardKey(rawCard)) {
    return NextResponse.json({ ok: false, error: '유효하지 않은 cardKey입니다.' }, { status: 400 })
  }
  if (rawSeason && !isValidSeason(rawSeason)) {
    return NextResponse.json({ ok: false, error: '유효하지 않은 season입니다.' }, { status: 400 })
  }

  const items = listHomeHubCandidates({
    cardKey: rawCard || undefined,
