...
export async function GET(_request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    const row = await prisma.bongFood.findUnique({ where: { id }, select: foodSelect })
    if (!row) return NextResponse.json({ error: '찾을 수 없습니다.' }, { status: 404 })
    return NextResponse.json(row)
  } catch (e) {
    console.error('[bong-foods/[id] GET]', e)
...
export async function PATCH(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  if ('description' in b) data.description = typeof b.description === 'string' ? b.description : null
  if ('category' in b) data.category = typeof b.category === 'string' ? b.category : null
  ...
  try {
    const row = await prisma.bongFood.update({
      where: { id },
      data: data as {
        name?: string
    ...
    console.error('[bong-foods/[id] PATCH]', e)
...
export async function DELETE(_request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const { id } = await context.params
  try {
    await prisma.bongFood.delete({ where: { id } })
    return NextResponse.json({ ok: true })
  } catch (e) {
    const code = (e as { code?: string })?.code
    if (code === 'P2025') return NextResponse.json({ error: '찾을 수 없습니다.' }, { status: 404 })
    console.error('[bong-foods/[id] DELETE]', e)
