...
export async function POST(req: Request) {
  ...
  if (!admin) return jsonWithLeakGuard({ error: "unauthorized" }, "admin.bongsim.coupons.bulk", { status: 401 });

  let body: BulkBody;
  try {
    body = (await req.json()) as BulkBody;
  } catch {
    return jsonWithLeakGuard({ error: "invalid_json" }, "admin.bongsim.coupons.bulk", { status: 400 });
  }

  const prefixRaw = typeof body.prefix === "string" ? body.prefix.trim() : "";
  if (!/^[A-Za-z0-9]+$/.test(prefixRaw)) {
    return jsonWithLeakGuard({ error: "invalid_prefix" }, "admin.bongsim.coupons.bulk", { status: 400 });
  }
  const count = typeof body.count === "number" && Number.isFinite(body.count) ? Math.trunc(body.count) : Number.NaN;
  if (!Number.isFinite(count) || count < 1 || count > 500) {
    return jsonWithLeakGuard({ error: "invalid_count" }, "admin.bongsim.coupons.bulk", { status: 400 });
  }
  ...
  if (!Number.isFinite(dv) || dv < 0) {
    return jsonWithLeakGuard({ error: "invalid_discount_value" }, "admin.bongsim.coupons.bulk", { status: 400 });
  }
  const max_disc =
  ...
  if (Number.isNaN(valid_from.getTime()) || Number.isNaN(valid_until.getTime()) || valid_until <= valid_from) {
    return jsonWithLeakGuard({ error: "invalid_dates" }, "admin.bongsim.coupons.bulk", { status: 400 });
  }
  const description =
    typeof tpl.description === "string" && tpl.description.trim() ? tpl.description.trim().slice(0, 500) : null;

  const pool = getPgPool();
  if (!pool) return jsonWithLeakGuard({ error: "db_unconfigured" }, "admin.bongsim.coupons.bulk", { status: 503 });
  ...
  try {
    ...
    for (let i = 0; i < count; i++) {
      ...
      if (!inserted) {
        ...
        return jsonWithLeakGuard({ error: "unique_collision_exhausted" }, "admin.bongsim.coupons.bulk", { status: 409 });
      }
    }
    ...
    const err = e as { code?: string };
    if (err.code === "42P01") return jsonWithLeakGuard({ error: "coupon_table_missing" }, "admin.bongsim.coupons.bulk", { status: 503 });
    console.error("[admin/bongsim/coupons/bulk]", e);
    return jsonWithLeakGuard({ error: "insert_failed" }, "admin.bongsim.coupons.bulk", { status: 500 });
  } finally {
    client.release();
  }

  return jsonWithLeakGuard({ ok: true, created: codes.length, codes }, "admin.bongsim.coupons.bulk.response");
}
