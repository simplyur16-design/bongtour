...
export default function AdminBongTipsListPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/admin/marketing/bong-tips?${q}`)
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-6">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          <h1 className="text-xl font-semibold text-bt-title">봉 팁</h1>
          <p className="mt-1 text-sm text-bt-body/70">국가·도시 단위 여행 팁</p>
        </div>
        <Link
          href="/admin/marketing/bong-tips/new"
      ...
      <div className="overflow-x-auto rounded-lg border border-bt-border-strong bg-white shadow-sm">
        <table className="min-w-full text-left text-sm">
          ...
          <tbody>
            {loading ? (
              <tr>
                <td colSpan={7} className="px-4 py-8 text-center text-bt-body/60">
                  불러오는 중…
                </td>
              </tr>
            ) : items.length === 0 ? (
              ...
              items.map((row) => (
                <tr key={row.id} className="border-b border-gray-100 hover:bg-gray-50/80">
                  ...
                  <td className="px-4 py-2 text-xs">{row.tipKind ?? '—'}</td>
                  ...
                  <td className="px-4 py-2">
                    <Link href={`/admin/marketing/bong-tips/${row.id}`} className="text-bt-brand-blue hover:underline">
                      편집
                    </Link>
                  </td>
                </tr>
