...
export default function EmailRegisterForm() {
  ...
  useEffect(() => {
    const mc = searchParams?.get('marketingConsent')?.trim().toLowerCase()
    if (mc === '1' || mc === 'true') setMarketingConsent(true)
  }, [searchParams])
      ...
      <div className="rounded-lg border border-slate-200 bg-white p-3">
        ...
        {privacyOpen ? (
          <div className="mt-2 space-y-2 rounded-md border border-slate-200 bg-slate-50 p-3 text-xs leading-relaxed text-slate-700">
            ...
            <p>
              1. 수집 항목: 이메일, 비밀번호(암호화 저장), 연락처, 서비스 이용기록/접속 로그/IP/기기 정보 등
            </p>
