import {
  MY_ESIM_BADGE,
  type MyEsimBadgeTier,
} from '@/src/constants/my-esim-design';
...
export function buildUsageSummaryView(
  ...
  if (usage.unlimited) {
    const todayLabel = formatDataAmount(todayUsageMb(usage.history));
    return {
...
export function chartBarHeight(usageMb: number, maxMb: number): number {
  return Math.max(8, Math.round((usageMb / Math.max(1, maxMb)) * 88));
}
