/** [lottetour] register-flight-evidence-supplier */
/**
...
/** 롯데관광: 공항명 줄·시각 줄이 분리된 경우 인접 줄을 묶어 evidence로 쓴다 */
function lottetourAirportWithAdjacentTime(
  block: string[],
  which: 'first' | 'last',
...
function lottetourSnippet(
  ...
  return trySources(parsed, preview, (lines) => {
    ...
    const depIncheon = lottetourAirportWithAdjacentTime(depBlock, 'first', (l) => l.includes('인천'))
    const arrIncheon = lottetourAirportWithAdjacentTime(arrBlock, 'last', (l) => l.includes('인천'))
...
export function buildSupplierFlightSnippet(
  ...
  else if (b === 'lottetour') raw = lottetourSnippet(kind, parsed, preview)
