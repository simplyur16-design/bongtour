/**
 * 롯데관광(lottetour) 등록 detail-body: 정규화·앵커·섹션 분리·슬라이스.
 *
 * `flight_section` / `optional_tour_section` / `shopping_section` 앵커는 **일정·호텔·포함불포함과의 경계**를 잡기 위한 것이며,
 * 이 축의 **표·항공 구조화**는 하지 않는다(SSOT: `register-input-parse-lottetour` + 정형 입력란).
 *
...
/** 롯데관광 상세 본문·일정 블록 상단 정렬에 맞춘 앵커 */
const LOTTETOUR_SECTION_ANCHOR_ALIASES: Record<DetailSectionType, string[]> = {
  summary_section: [
    '여행핵심정보',
    '상품 핵심정보',
    '여행 주요일정',
...
function buildLottetourAnchorStarters(): Array<{ type: DetailSectionType; re: RegExp }> {
  return (Object.keys(LOTTETOUR_SECTION_ANCHOR_ALIASES) as DetailSectionType[]).map((type) => ({
    type,
    re: new RegExp(
      `(${LOTTETOUR_SECTION_ANCHOR_ALIASES[type]
        .slice()
...
const LOTTETOUR_STARTERS = buildLottetourAnchorStarters()

function isFalseShoppingAnchorLineLottetour(line: string): boolean {
...
function isFalseHotelAnchorLineLottetour(line: string): boolean {
  const t = line.replace(/\s+/g, ' ').trim()
...
function splitLottetourSectionsByAnchors(normalized: string): Array<{ type: DetailSectionType; text: string }> {
  ...
  for (const line of lines) {
    const hit = LOTTETOUR_STARTERS.find((s) => {
      const t = line.trim()
      if (s.type === 'hotel_section') {
        if (/^호텔$/i.test(t)) return true
        if (!s.re.test(line)) return false
        if (isFalseHotelAnchorLineLottetour(line)) return false
        return true
      }
      if (!s.re.test(line)) return false
      if (s.type === 'shopping_section' && isFalseShoppingAnchorLineLottetour(line)) return false
      return true
    })
    if (hit && buf.length > 0) {
      flush()
...
/** 항공 시드: flight 블록(join) → schedule → summary 순으로 이어 붙임. 하한 72. (일정이 요약보다 앞선 붙여넣기를 가정 — `docs/body-parser-lottetour-ssot.md` 항공 시드 슬라이스와 동일.) */
const LOTTETOUR_FLIGHT_SEED_MIN_LEN = 72
...
export function sliceDetailBodySections(
  ...
  const flightSectionSeed = [pick('summary_section'), joinSections('flight_section'), pick('schedule_section')]
    ...
    .join('\n')
  const flightSection =
    flightSectionSeed.length < LOTTETOUR_FLIGHT_SEED_MIN_LEN ? normalizedRaw : flightSectionSeed
  return {
    flightSection,
...
export function normalizeDetailRawText(raw: string): string {
  ...
  const dropLine = (t: string) =>
    /^(더보기|크게보기|후기|리뷰|좋아요|공유|배너|이벤트|버튼|[-_=]{3,}|롯데관광\s*고객|옐로우\s*볼)$/i.test(
      t.replace(/\s+/g, ' ').trim()
    )
  return raw
    .replace(/\r/g, '\n')
...
export function splitDetailSections(normalized: string): Array<{ type: DetailSectionType; text: string }> {
  return splitLottetourSectionsByAnchors(normalized)
}

/** 레거시·비등록 용도. 관리자 등록 SSOT는 `detail-body-parser-lottetour` 전용 포함/불포함 파서를 쓴다. */
