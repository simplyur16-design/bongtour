/**
 * 롯데관광(lottetour) 전용 — structured tour signals (선택관광·쇼핑 등 본문 신호 추출).
 *
 * Gate 연결
 * - 선택관광 행 필터/금칙은 `@/lib/optional-tour-row-gate-lottetour`만 import한다. 공용 `optional-tour-row-gate.ts`는 없다.
...
import { filterOptionalTourRows, isBannedOptionalTourName } from '@/lib/optional-tour-row-gate-lottetour'
...
function extractBestPriceFromOptionalCell(cell: string | null | undefined): number | null {
  ...
  const t = cell.trim()
  const euroSym = t.match(/€\s*([0-9]{1,3}(?:,[0-9]{3})*|[0-9]{2,6})\b/)
  if (euroSym) {
    const n = Number(euroSym[1]!.replace(/,/g, ''))
    if (Number.isFinite(n) && n >= 1) return n
  }
  const euroWord = t.match(/(\d[\d,]*)\s*유로\b/i)
  if (euroWord) {
    const n = Number(euroWord[1]!.replace(/,/g, ''))
    if (Number.isFinite(n) && n >= 1) return n
  }
