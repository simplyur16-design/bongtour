...
} from '@/lib/register-llm-schema-naeiltour'

/** naeiltour 등록 save 게이트: 달력 행만이 아니라 표·항공 구조화·일정 초안을 함께 본다. */
...
import {
  enrichNaeiltourDepartureInputsForConfirmSave,
  loadNaeiltourProductInfantFallback,
  parsedPricesToDepartureInputs,
  upsertProductDepartures,
  type DepartureInput,
} from '@/lib/upsert-product-departures-naeiltour'
import { toDeparturePreviewRows } from '@/lib/departure-preview'
import {
  upsertItineraryDays,
  registerScheduleToDayInputs,
  type ItineraryDayInput,
} from '@/lib/upsert-itinerary-days-naeiltour'
...
} from '@/lib/price-promotion-naeiltour'
import { issuePreviewToken, verifyPreviewToken } from '@/lib/registration-preview-token'
import { departureInputsToProductPriceCreateMany } from '@/lib/product-departure-to-price-rows-naeiltour'
import { dedupeNaeiltourProductPriceCreateRows } from '@/lib/naeiltour-product-price-create-rows'
...
import { mergeDayHotelPlansForRegister } from '@/lib/day-hotel-plans-naeiltour'
...
import { resolveNaeiltourHeroTripDates } from '@/lib/product-hero-naeiltour'
...
} from '@/lib/flight-manual-correction-naeiltour'
import { buildRegisterPreviewCanonicalString } from '@/lib/register-preview-content-fingerprint-naeiltour'
import {
  REGISTER_ADMIN_SNAPSHOT_STATUS,
  type RegisterAdminSnapshotStatus,
} from '@/lib/register-admin-audit-status-naeiltour'
import { nextRegisterAnalysisAttemptNo } from '@/lib/register-admin-analysis-store-naeiltour'
import {
...
} from '@/lib/register-admin-input-persist-naeiltour'
import { tryLoadRegisterParsedForConfirmReuse } from '@/lib/register-admin-confirm-reuse-naeiltour'
import { buildRegisterProductScheduleJson } from '@/lib/build-register-product-schedule-json'
import { buildRegisterVerificationBundle } from '@/lib/admin-register-verification-meta-naeiltour'
import type { RegisterPreviewProductDraft } from '@/lib/register-preview-payload-naeiltour'
...
import { mergeNaeiltourDeterministicFieldsFromPaste } from '@/lib/naeiltour-paste-deterministic-patch'
import { extractNaeiltourTripAnchorsFromPaste } from '@/lib/naeiltour-trip-anchors-from-paste'
import { naeiltourBuildMinimalDepartureInputs } from '@/lib/naeiltour-synthetic-departure'

type HeroTripDatesSupplement = Partial<Pick<Parameters<typeof resolveNaeiltourHeroTripDates>[0], 'naeiltourFlightStructured'>>
...
/** naeiltour 등록 preview/confirm 전용 오케스트레이션 — `handleParseAndRegisterNaeiltourRequest`만 연결. */
let currentLogPrefix = '[parse-and-register]'
const isDev = process.env.NODE_ENV === 'development'

/**
 * 내일투어 전용: 단계별 경과 시간을 항상 서버 로그에 남김(개발·운영 공통).
 * `parse-and-register-timing`은 dev에서만 출력하므로, 4분 지연 구간 특정용으로 별도 사용.
 */
function createNaeiltourRegisterTiming(logPrefix: string) {
  const t0 = Date.now()
  const devPrefix = `${logPrefix}[timing]`
  function mark(label: string) {
    if (process.env.NAEILTOUR_REGISTER_TIMING_LOG !== '1') return
    const elapsed = Date.now() - t0
    console.info(`[naeiltour][timing] ${label} +${elapsed}ms`)
    if (isDev) {
...
export type ParseAndRegisterFlowOptions = {
  /** 라우트 SSOT — 본 파일은 항상 `'naeiltour'`만 허용, body.brandKey는 검증용(선택). */
  ...
  /**
   * augmentParsed 이후 한 번 더 패치(예: 내일투어 basicAjax 상세카드). 브랜드 문자열 분기 없음 — 핸들러가 구현을 넘긴다.
   */
  ...
  /** `resolveNaeiltourHeroTripDates`에 합쳐 넣을 추가 인자(구조화 항공 blob 등). */
...
function computePreviewContentDigestForBody(body: Record<string, unknown>): string {
  ...
  const brandKey = 'naeiltour' as const
...
export async function runNaeiltourRegisterFlow(request: Request, flowOptions: ParseAndRegisterFlowOptions) {
  ...
  try {
    ...
    assertRegisterRouteSupplierMatch('naeiltour', body.originSource, {
      route: '/api/travel/parse-and-register-naeiltour',
    })
    const text = typeof body.text === 'string' ? body.text.trim() : ''
    const brandKeyFromBody = typeof body.brandKey === 'string' ? body.brandKey.trim() || null : null
    if (forcedBrandKey !== 'naeiltour') {
      return NextResponse.json({ error: '내부 설정 오류: naeiltour 전용 오케스트레이션입니다.' }, { status: 500 })
    }
    if (
      brandKeyFromBody &&
      normalizeBrandKeyToCanonicalSupplierKey(brandKeyFromBody) !== 'naeiltour'
    ) {
      return NextResponse.json(
        {
          error: '요청 brandKey와 엔드포인트가 맞지 않습니다. 이 API에는 brandKey "naeiltour"만 허용됩니다.',
        },
        { status: 400 }
      )
    }
    const brandKey = 'naeiltour'
    ...
    if (!text && !hasParsed && !originUrl) {
      return NextResponse.json(
        {
          error:
            '내일투어 등록에는 공급사 상품 URL 또는 상세 본문 붙여넣기가 필요합니다.',
        },
        { status: 400 }
      )
    }
    ...
    if (patchParsedAfterAugment) {
      ...
      if (
        shouldSkipConfirmDetailPatch({
          ...
          detailCollectRan: parsed.naeiltourDetailCollectRan,
    ...
    const naeiltourAnchors = extractNaeiltourTripAnchorsFromPaste(
      text,
      parsed.detailBodyStructured?.flightStructured ?? null
    )
    if (naeiltourAnchors.goodCd && !String(parsed.goodCd ?? '').trim()) {
      parsed = { ...parsed, goodCd: naeiltourAnchors.goodCd }
    }
    if (naeiltourAnchors.eventSeq && !String(parsed.eventSeq ?? '').trim()) {
      parsed = { ...parsed, eventSeq: naeiltourAnchors.eventSeq }
    }
    if (departureInputs.length === 0 && naeiltourAnchors.tripStartIso) {
      departureInputs = applyDepartureTerminalMeetingInfo(
        naeiltourBuildMinimalDepartureInputs(naeiltourAnchors.tripStartIso, parsed, text)
      )
    }
    if (isDev && process.env.NAEILTOUR_REGISTER_PERF_LOG === '1') {
      console.info('[register-naeiltour]', {
        supplier: 'naeiltour',
        mode,
        saveReuse: hasParsed || reusedConfirmAnalysis,
        llmCalled: llmRanThisRequest,
        tripStartSource: naeiltourAnchors.tripStartSource,
        tripEndSource: naeiltourAnchors.tripEndSource,
    ...
    const heroResolvedPreview = resolveNaeiltourHeroTripDates({
      selectedDate: calendarDep,
      fallbackPriceRowDate: calendarDep,
      duration: parsed.duration,
      departureFacts: factsFromRow,
      naeiltourFlightStructured: heroTripDatesExtra.naeiltourFlightStructured ?? null,
    })
    ...
    const naeiltourFlightLegOk = Boolean(
      fs &&
    ...
    const naeiltourFlightStructuredOk =
      fsDbg?.status === 'success' || (fsDbg?.status === 'partial' && naeiltourFlightLegOk) || naeiltourFlightLegOk
    if (!flightHasUsableCore && !naeiltourFlightStructuredOk) {
      heroDateFieldIssues.push({
        field: 'flight_info',
    ...
    const { buildRegisterPreviewSsotMeta } = await import('@/lib/register-preview-ssot-naeiltour')
    ...
    const autoExtracted = {
      ...
      originUrl,
      adapterPrefetchRan: Boolean(parsed.naeiltourDetailCollectRan),
      ...
      adapterSummaryPreview:
        parsed.naeiltourDetailCollectSummary ??
        (parsed.naeiltourDetailCollectRan === false
    ...
    if (mode === 'preview') {
      ...
      const { buildRegisterCorrectionPreview } = await import('@/lib/register-correction-preview-naeiltour')
      ...
      const registerVerification = buildRegisterVerificationBundle({
        ...
        route: '/api/travel/parse-and-register-naeiltour',
        handler: 'parse-and-register-naeiltour-handler',
        parsed: parsedForPreview,
        productDraft,
        fieldIssues: combinedFieldIssues,
      })
    ...
    const heroAuditForMeta = resolveNaeiltourHeroTripDates({
    ...
    const highlightPointsRaw = extractHighlightFromNaeiltour(text) ?? null
    ...
    const infantFallback =
      ...
      (await loadNaeiltourProductInfantFallback(prisma, productId))
    const departureInputsForSave = enrichNaeiltourDepartureInputsForConfirmSave(departureInputs, infantFallback)
    ...
    if (priceRows.length > 0) {
      // REGRESSION-FREEZE[naeiltour-register-api-price-inject]: dedupe ProductPrice by date — manifest
      priceRows = dedupeNaeiltourProductPriceCreateRows(priceRows)
