/**
 * 롯데관광(lottetour) 경로용 — 공개 상품 상세는 레거시 ProductPrice[] 를 직렬화해 쓰는데,
...
export type MergeProductPriceBodyTableOptions = {
  ...
  /**
   * 롯데관광(lottetour) 전용: 출발별 성인가가 다를 때 아동=행별 성인가 연동(post).
   * 본문 표의 유아 단가는 **절대** 행에 덮어쓰지 않는다(최초 유아 단가·행 값 유지).
   */
  lottetourVaryingAdultChildLinkage?: boolean
}
...
export function mergeProductPriceRowsWithBodyPriceTable(
  ...
  const varyingAdultChildLinkage =
    options?.modetourVaryingAdultChildLinkage === true || options?.lottetourVaryingAdultChildLinkage === true
  const mergeInfant =
    !options?.lottetourVaryingAdultChildLinkage &&
    table.infantPrice != null &&
    table.infantPrice > 0 &&
    !infantVaries &&
  ...
  if (varyingAdultPost) {
    ...
    merged = merged.map((r) => {
      ...
      const mirrorChildToAdult =
        (options?.lottetourVaryingAdultChildLinkage === true && ad > 0) ||
      ...
      if (
        options?.lottetourVaryingAdultChildLinkage !== true &&
        tin != null &&
        tin > 0 &&
...
export function departureInputsToProductPriceCreateMany(
  ...
  const toYmd = (d: DepartureInput): string =>
    d.departureDate instanceof Date
      ? d.departureDate.toISOString().slice(0, 10)
