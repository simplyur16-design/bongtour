// verygoodtour(참좋은여행) 전용 schedule extract 모듈
// lib/register-schedule-extract-common.ts에서 분리됨 (단계적 독립화 작업)
// 다른 공급사(modetour/hanatour/ybtour)는 각자 자기 파일 사용
/**
...
function parseScheduleRowsFromLlmJson(
  ...
  if (opts.strictDay != null) {
    ...
    if (row) return applyVerygoodScheduleImageKeywordsToRows([{ ...row, day: want }])
    return []
  }
  ...
  return applyVerygoodScheduleImageKeywordsToRows(out)
}
