/**
 * 교보이지(kyowontour) 전용: `Product.schedule[].imageKeyword`만 Pexels/이미지 검색용 영문 noun phrase로 정리.
...
export type KyowontourImageKeywordContext = {
  ...
  /** 일차 원문 블록(붙여넣기 파이프라인) */
  blob?: string
  /** 에어텔(항공+호텔) + 일정 빈약 시 도시 기반 키워드(교보이지 전용) */
...
/** 최소 한글→영문 관광지 (교보이지 일정 본문에서 자주 쓰이는 표기만) */
...
/** 에어텔 일정이 관광지 서술 없이 빈약한지(교보이지 전용) */
...
/** 교보이지 전용: 에어텔+빈약 일정용 도시/권역 키워드(타 공급사 미사용) */
