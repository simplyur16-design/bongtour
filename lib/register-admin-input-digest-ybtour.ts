/** [ybtour] register-admin-input-digest — preview↔confirm 입력 지문 */
import { createHash } from 'crypto'
import { buildRegisterPreviewCanonicalString } from '@/lib/register-preview-content-fingerprint-ybtour'
import {
  normalizeYbtourRegisterOriginUrl,
  parseYbtourRegisterPastedBlocksFromBody,
} from '@/lib/register-admin-core-ybtour'

/** @deprecated 이름 호환 — `parseYbtourRegisterPastedBlocksFromBody` */
export const parseRegisterPastedBlocksPayload = parseYbtourRegisterPastedBlocksFromBody
...
export function computeRegisterInputDigestFromBody(
  ...
  const originUrl = originUrlRaw && originUrlRaw !== '' ? normalizeYbtourRegisterOriginUrl(originUrlRaw) : null
  const travelScope = typeof body.travelScope === 'string' ? body.travelScope.trim() : ''
  const pb = parseYbtourRegisterPastedBlocksFromBody(body)
