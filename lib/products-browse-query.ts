...
/** 구 URL·사이드바 호환 — 항공+호텔·프리미엄 키워드 필터 */
export type ProductCategoryFilter = 'air-hotel' | 'premium'
...
function parseCategories(raw: string | null, repeated: string[]): ProductCategoryFilter[] {
  ...
  for (const r of [...parseMulti(raw), ...repeated]) {
    ...
    if (u === 'airtel' || u === 'air-hotel' || u === 'free') set.add('air-hotel')
    else if (u === 'premium') set.add('premium')
  }
  return [...set]
}
