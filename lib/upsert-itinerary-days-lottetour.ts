/**
 * ItineraryDay: 롯데관광(lottetour) 등록/저장 경로용 — 공급사 원문 일정표 정본(raw/source-of-truth) 계층.
...
export function registerScheduleToDayInputs(
  ...
  return schedule.flatMap((s): ItineraryDayInput[] => {
    ...
    const headerTitle =
      headerTitleRaw && !shouldReplaceLottetourScheduleDayTitle(headerTitleRaw, description)
        ? headerTitleRaw
        : deriveLottetourScheduleDayHeaderTitle({
            day,
