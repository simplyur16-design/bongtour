 ...
 * 내일투어: URL·hidden에서 goodCd·eventSeq.
 */
import type { FlightStructured } from '@/lib/detail-body-parser-types'
import { normalizeCalendarDate } from '@/lib/date-normalize'
import { extractNaeiltourMasterIdsFromBlob } from '@/lib/naeiltour-paste-deterministic-patch'

export type NaeiltourTripAnchors = {
...
export function extractNaeiltourTripAnchorsFromPaste(
  blob: string,
  fs: FlightStructured | null | undefined,
): NaeiltourTripAnchors {
  const norm = blob.replace(/\r/g, '\n')
  const master = extractNaeiltourMasterIdsFromBlob(norm)
  ...
  return {
    ...
    tripStartSource,
    tripEndSource,
    goodCd: master.goodCd,
    eventSeq: master.eventSeq,
  }
}
