/**
 * 하나투어 등록 detail-body: 정규화·앵커·섹션 분리·슬라이스·일정 원료 thinning.
 *
 * 항공/옵션/쇼핑 앵커는 **섹션 경계**용이다. 해당 축 **구조화**는 `register-input-parse-hanatour` + 정형 입력란 SSOT.
 *
 * @see docs/body-parser-hanatour-ssot.md
 *
 * 공통 등록 정책: `docs/admin-register-supplier-precise-spec.md`. 일정 표현: `docs/register_schedule_expression_ssot.md`.
 */
import type {
  DetailSectionType,
...
/** 하나투어 붙여넣기에 자주 나오는 상단·푸터 노이즈 (다른 공급사와 별도 유지) */
const HANATOUR_SECTION_ANCHOR_ALIASES: Record<DetailSectionType, string[]> = {
  summary_section: [
    '여행핵심정보',
    '상품 핵심정보',
    '여행 주요일정',
...
/** 레거시·비등록 용도. 관리자 등록 SSOT는 `detail-body-parser-hanatour`의 전용 포함/불포함 파서를 쓴다. */
export function parseIncludedExcludedSection(section: string): IncludedExcludedStructured {
  ...
  const excludedItems = lines.filter((l) => /(불포함|excluded|별도)/i.test(l)).slice(0, 30)
  return {
    includedItems,
    excludedItems,
