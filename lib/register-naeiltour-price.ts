/**
 * 내일투어 등록 가격: 3슬롯(성인·아동·유아). 쿠폰·총액·잔여석·적립·무이자 등 메타는 슬롯 제외.
 */
...
function lineIsNaeiltourMeta(line: string): boolean {
...
function detectTier(line: string): Tier | null {
  ...
  if (!s || lineIsNaeiltourMeta(s)) return null
...
export type NaeiltourThreeSlotExtract = {
  adultPrice: number | null
  childPrice: number | null
  infantPrice: number | null
}

export function extractNaeiltourThreeSlotPricesFromBlob(blob: string): NaeiltourThreeSlotExtract | null {
  if (!blob?.trim()) return null
  const lines = blob.replace(/\r/g, '\n').split('\n')
  const out: NaeiltourThreeSlotExtract = { adultPrice: null, childPrice: null, infantPrice: null }
...
function naeiltourExtraPriceBlobFromDetailBody(parsed: RegisterParsed): string {
...
function naeiltourPriceBlobFromParsed(parsed: RegisterParsed): string {
  ...
  return [
    ...
    naeiltourExtraPriceBlobFromDetailBody(parsed),
    head,
  ]
    .filter((x) => x.length > 0)
    .join('\n\n')
...
export function finalizeNaeiltourProductPriceTable(
  table: RegisterParsed['productPriceTable'] | null | undefined,
  blob: string
): RegisterParsed['productPriceTable'] | null {
  const fromBlob = extractNaeiltourThreeSlotPricesFromBlob(blob)
  const t = table ?? null
...
export function finalizeNaeiltourRegisterParsedPricing(parsed: RegisterParsed): RegisterParsed {
  const blob = naeiltourPriceBlobFromParsed(parsed)
  const next = finalizeNaeiltourProductPriceTable(parsed.productPriceTable ?? null, blob)
