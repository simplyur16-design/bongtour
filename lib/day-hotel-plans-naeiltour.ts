/**
 * 내일투어(naeiltour) 경로용 — 일차별 예정호텔(1일차 예정호텔 …) 구조. 상세 호텔정보 탭 SSOT.
 * DB 필드명은 Prisma와 무관하게 rawMeta.structuredSignals + 서버 병합으로 전달.
 */

export const NAEILTOUR_HOTEL_UNDECIDED_PLACEHOLDER =
  '현재 숙박시설은 미정입니다. 출발 전 안내됩니다.'

const NAEILTOUR_REVIEW_OR_PROSE_LIKE =
...
export function naeiltourHotelLineLooksUndecidedOnly(line: string): boolean {
  const t = line.replace(/\r/g, '').trim()
  if (!t) return true
  if (NAEILTOUR_REVIEW_OR_PROSE_LIKE.test(t) && t.length > 40) return false
...
export function naeiltourHotelTextBlockIsUndecidedOnly(text: string | null | undefined): boolean {
  ...
  return lines.every((l) => naeiltourHotelLineLooksUndecidedOnly(l))
}

/** 일정표의 매일 hotelText가 모두 “미정” 수준이면 본문에서 호텔 블록을 억지로 파싱하지 않는다. */
export function naeiltourScheduleHotelsAreAllUndecided(
  ...
  for (const s of schedule) {
    ...
    if (!ht) return false
    if (!naeiltourHotelTextBlockIsUndecidedOnly(ht)) return false
  }
  return true
}

/** 리뷰·장문 본문이 호텔 플랜으로 잘못 들어온 경우 걸러낸다. */
export function naeiltourDayHotelPlansLookLikeRealHotels(plans: DayHotelPlan[]): boolean {
  ...
  for (const p of plans) {
    for (const h of p.hotels ?? []) {
      ...
      if (NAEILTOUR_REVIEW_OR_PROSE_LIKE.test(t)) return false
      if (naeiltourHotelLineLooksUndecidedOnly(t)) continue
      if (t.length >= 2 && t.length <= 120) sawReal = true
    }
    const raw = (p.raw ?? '').trim()
    if (raw.length > 200 && NAEILTOUR_REVIEW_OR_PROSE_LIKE.test(raw)) return false
  }
  return sawReal
}

export function naeiltourPlaceholderDayHotelPlans(): DayHotelPlan[] {
  return [{ dayIndex: 1, label: '숙박 안내', hotels: [NAEILTOUR_HOTEL_UNDECIDED_PLACEHOLDER] }]
}
...
export function dayHotelPlansFromSchedule(
  ...
  for (const s of schedule) {
    const ht = s.hotelText?.trim()
    if (!ht) continue
    if (naeiltourHotelTextBlockIsUndecidedOnly(ht)) continue
...
export function resolveDayHotelPlansForPublic(
  ...
  const schedAllUndecided = naeiltourScheduleHotelsAreAllUndecided(schedule)

  const fromStruct = normalizeDayHotelPlansFromUnknown(structuredPlans ?? [])
  if (fromStruct.length) {
    if (naeiltourDayHotelPlansLookLikeRealHotels(fromStruct)) return fromStruct
  ...
  if (schedAllUndecided) return naeiltourPlaceholderDayHotelPlans()

  const blob = [hotelInfoRaw, hotelSummaryRaw].filter((x) => x?.trim()).join('\n\n')
  const parsed = parseDayHotelPlansFromSupplierText(blob)
  if (parsed.length && naeiltourDayHotelPlansLookLikeRealHotels(parsed)) return parsed
  return []
}
...
export function mergeDayHotelPlansForRegister(
  ...
  if (fromLlm.length) {
    if (naeiltourDayHotelPlansLookLikeRealHotels(fromLlm)) return fromLlm
