/**
 * 하나투어 전용 — Gemini로 상품 핵심 포인트 영역 추출·정리.
...
const HANATOUR_SYSTEM = `당신은 하나투어 상세 붙여넣기 원문에서 특정 구역만 분리하는 도우미입니다.
반드시 JSON 객체만 출력합니다. 마크다운·코드펜스·주석 금지.

하나투어 규칙:
- 추출 대상은 「📌 상품 핵심 포인트」「상품 핵심 포인트」 등 📌·핵심 포인트로 표시된 블록만입니다.
- 「하나팩」「HANA PACK」「패키지 등급」「등급 라벨」류의 패키지 등급·마케팅 라벨 줄은 highlightPointsRaw에 포함하지 마세요.
- 원문에 해당 블록이 없으면 두 필드 모두 null.

환각 방지:
- 원문에 없는 내용 추가 금지.
- highlightPointsRaw는 공급사 문구를 유지하고 태그만 정리 가능.
- highlightPoints는 봉투어 톤으로 다듬되 새 정보 금지. 5~7줄, 한 줄 핵심 1개.
`
...
export async function extractHighlightFromHanatourLLM(rawText: string): Promise<{
  highlightPointsRaw: string | null
  highlightPoints: string | null
} | null> {
...
`
  ...
  const model = genAI.getGenerativeModel({
    model: getModelName(),
    systemInstruction: HANATOUR_SYSTEM,
  })
  ...
  try {
    ...
    console.warn('[llm-extract-highlight-hanatour]', e instanceof Error ? e.message : e)
    return null
  }
}
