...
type AdminSecretGlobal = typeof globalThis & {
  __bongtourWarnedLegacyDevBypass?: boolean
  __bongtourWarnedLegacyBearer?: boolean
}

const ag = globalThis as AdminSecretGlobal
...
export function getDevAdminBypassSecret(): string {
  ...
  if (legacy && process.env.NODE_ENV === "development" && !ag.__bongtourWarnedLegacyDevBypass) {
    ag.__bongtourWarnedLegacyDevBypass = true
    console.warn(
