/**
 * 참좋은여행(verygoodtour) 등록 일정 표현 SSOT — routeText(a–g ` - `) · description(공급사 문장 우선, 없으면 route 명소 2~3문장).
 * REGRESSION-FREEZE[verygoodtour-schedule-expression]: routeText·description vibe — manifest
 * REGRESSION-FREEZE[verygoodtour-schedule-expression]: PackageDetail itinerary — manifest
 */
...
const VERYGOODTOUR_ROUTE_PLACE_NOISE_RE =
...
const VERYGOODTOUR_ROUTE_LABEL_TRIM_RE =
  /(?:으로?\s*이동|으로?\s*출발|으로?\s*귀국|로\s*이동|방문|관광|투어|탐방|체험|승차|하차|탑승|도착|출발|미팅|피켓|조식\s*후|중식\s*후|석식\s*후)$/u

function cleanVerygoodtourRoutePlaceLabel(raw: string): string {
  return String(raw ?? '')
    .replace(/^#+\s*/, '')
    .replace(/^■+\s*/, '')
    .replace(/^[\s·▪▶●\-–—]+/, '')
    ...
    .replace(/\s+/g, ' ')
    .replace(VERYGOODTOUR_ROUTE_LABEL_TRIM_RE, '')
    .trim()
}
...
function isVerygoodtourRoutePlaceNoise(label: string): boolean {
  ...
  if (VERYGOODTOUR_ROUTE_PLACE_NOISE_RE.test(t)) return true
...
function normalizeVerygoodtourRoutePlaceKey(label: string): string {
  return label
    .toLowerCase()
    .replace(/\s*\([^)]*\)\s*/g, ' ')
    .replace(/[^a-z0-9가-힣]/g, '')
...
export function dedupeVerygoodtourScheduleRoutePlaces(places: readonly string[]): string[] {
  const out: string[] = []
  const keys: string[] = []
  for (const raw of places) {
    const candidates = new Set<string>()
    const fromTitle = extractPlaceFromVerygoodtourTmTitle(String(raw ?? ''))
    ...
    for (const candidate of candidates) {
      const label = cleanVerygoodtourRoutePlaceLabel(candidate)
      if (!label || isVerygoodtourRoutePlaceNoise(label)) continue
      const key = normalizeVerygoodtourRoutePlaceKey(label)
      if (!key) continue
...
export function joinVerygoodtourScheduleRouteText(places: readonly string[], max = VERYGOODTOUR_SCHEDULE_ROUTE_MAX): string | null {
  return sanitizeRegisterScheduleRouteText(
    dedupeVerygoodtourScheduleRoutePlaces(places).slice(0, max).join(' - '),
    max,
  )
}

function extractPlaceFromVerygoodtourTmTitle(title: string): string | null {
  const t = String(title ?? '').trim()
  if (!t || isVerygoodtourRoutePlaceNoise(t)) return null
  const arrow = t.match(/(?:▶|●)\s*(.+)/)
  if (arrow?.[1]) {
    const label = cleanVerygoodtourRoutePlaceLabel(arrow[1])
    return label && !isVerygoodtourRoutePlaceNoise(label) ? label : null
  }
  const tour = t.match(/^(.{2,48}?)\s*(?:관광|방문|체험|투어|탐방)/u)
  if (tour?.[1]) {
    const label = cleanVerygoodtourRoutePlaceLabel(tour[1])
    return label && !isVerygoodtourRoutePlaceNoise(label) ? label : null
  }
  if (t.length >= 2 && t.length <= 48) {
    const label = cleanVerygoodtourRoutePlaceLabel(t)
    return label && !isVerygoodtourRoutePlaceNoise(label) ? label : null
  }
  return null
}
...
export function extractVerygoodtourSchedulePlacesFromCityLabels(cities: readonly string[]): string[] {
  return dedupeVerygoodtourScheduleRoutePlaces(cities)
}

/** 붙여넣기 블록 — ▶·불릿 줄 순서로 routeText 후보 */
export function extractVerygoodtourSchedulePlacesFromPastedBlock(block: string): string[] {
  const out: string[] = []
  for (const line of block.split(/\r?\n/)) {
    ...
    if (markdownCity?.[1]) {
      const label = cleanVerygoodtourRoutePlaceLabel(markdownCity[1])
      if (label && !isVerygoodtourRoutePlaceNoise(label)) out.push(label)
      continue
    }
    const bullet = t.match(/^(?:▶|●|-\s*)\s*(.+)/)
    if (bullet?.[1]) {
      const label = cleanVerygoodtourRoutePlaceLabel(bullet[1])
      if (label && !isVerygoodtourRoutePlaceNoise(label)) out.push(label)
      continue
    }
    const fromTitle = extractPlaceFromVerygoodtourTmTitle(t)
    if (fromTitle) {
      out.push(fromTitle)
      continue
    }
  ...
  return dedupeVerygoodtourScheduleRoutePlaces(out)
}

type VerygoodtourScheduleVibeProfile =
  | 'return_calm'
  | 'return_transit'
  | 'arrival'
  | 'macau_daytrip'
...
const VERYGOODTOUR_SCHEDULE_VIBE_DESCRIPTIONS: Record<VerygoodtourScheduleVibeProfile, readonly string[]> = {
  hk_walking: [
...
function inferVerygoodtourScheduleVibeProfile(day: number, maxDay: number, joinedBlob: string): VerygoodtourScheduleVibeProfile {
...
function verygoodtourHighlightLeakChunks(label: string): string[] {
...
export function applyVerygoodtourScheduleExpressionToRows<T extends RegisterScheduleDay>(rows: T[]): T[] {
  ...
  return rows.map((row) => {
    ...
    const fromExisting = row.routeText
      ...
      : []
    const fromPaste = extractVerygoodtourSchedulePlacesFromPastedBlock(String(row.description ?? ''))
    const routePlaces =
...
/** @deprecated SSOT 이름 — `applyVerygoodtourScheduleExpressionToRows` */
export const applyVerygoodScheduleExpressionToRows = applyVerygoodtourScheduleExpressionToRows

/** @deprecated SSOT 이름 — `verygoodtourFactDaysToRegisterSchedule` */
export const verygoodFactDaysToRegisterSchedule = verygoodtourFactDaysToRegisterSchedule
