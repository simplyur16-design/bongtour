/**
 * 교원이지: 옵션 블록·메타가 쇼핑 행으로 승격되지 않게 필터 + 쇼핑 횟수 정합.
...
export function parseKyowontourShoppingPasteTab(raw: string): ShoppingStructured | null {
...
function parseKyowontourShoppingVisitCount(hay: string): number | null {
...
export function sanitizeKyowontourShoppingStructured(
  ...
  const vc = parseKyowontourShoppingVisitCount(shoppingSection)
