...
export function logLottetourBasic(phase: string, payload: Record<string, unknown>) {
  console.log(`${PREFIX} phase=${phase} ${compactJson(payload)}`)
}

/** 본문 상단 `상품번호EWP…` / `상품번호 EWP…` */
export function extractLottetourProductCodeFromBlob(blob: string): string | null {
...
function isLottetourGlobalStopLine(t: string): boolean {
  if (!t) return true
...
function startsLottetourExtraChargeBlock(t: string): boolean {
  return (
    /^▷/.test(t) ||
    /^▼|^►/.test(t) ||
    /^♣/.test(t) ||
...
function isLottetourOptionalTourPitchLine(t: string): boolean {
...
function collectLottetourSurchargeChunk(lines: string[], startIdx: number): { consumed: number; summary: string | null } {
  let j = startIdx
  const buf: string[] = []
  const maxScan = 45
  let n = 0
  while (j < lines.length && n < maxScan) {
    ...
    if (isLottetourGlobalStopLine(t) && buf.length > 0) break
 ...
 */
export function parseLottetourIncludedExcludedSection(section: string): IncludedExcludedStructured {
  ...
  while (i < lines.length) {
    ...
    if (isLottetourGlobalStopLine(line) && mode !== 'idle') {
      mode = 'idle'
      i++
      continue
    }
    ...
    if (mode === 'inc') {
      ...
      if (isLottetourIncExcBulletLine(line)) {
    ...
    if (mode === 'exc') {
      ...
      if (isLottetourOptionalTourPitchLine(line)) {
        i++
        continue
      }
      if (startsLottetourExtraChargeBlock(line)) {
  ...
  return {
    ...
    noteText,
    reviewNeeded,
    reviewReasons: reviewNeeded ? ['lottetour_include_exclude_bullet_parse_empty'] : [],
  }
}
...
export function logLottetourBasicDetailBody(detail: DetailBodyParseSnapshot, rawLen: number) {
  ...
  if (codes.length) {
    logLottetourBasic('detail-body-flags', { failCodes: codes })
  }
}

/** 붙여넣기·URL에서 행사코드(evtCd). 예: `?evtCd=E01A260624KE007` */
export function extractLottetourEvtCdFromBlob(blob: string): string | null {
  const u = blob.match(/[?&]evtCd=([^&\s#'"<>]+)/i)
  if (u?.[1]) return u[1]!.trim()
  const m = blob.match(/\bevtCd\s*[:：=]\s*([A-Za-z0-9_-]{8,34})\b/i)
  return m?.[1]?.trim() ?? null
}
...
/** LLM JSON에 비어 있을 때 원문에서 godId·evtCd만 보강 */
export function mergeLottetourMasterIdsFromBlob(parsed: RegisterParsed, blob: string): RegisterParsed {
  let next = parsed
  const gid = extractLottetourGodIdFromBlob(blob)
  if (gid && !String(next.godId ?? '').trim()) {
    next = { ...next, godId: gid }
  }
...
export function applyLottetourStructuredPreviewFields(parsed: RegisterParsed): RegisterParsed {
...
export function logLottetourBasicRegisterFinal(parsed: RegisterParsed, rawTextLen: number) {
  const pt = parsed.productPriceTable
  const fs = parsed.detailBodyStructured?.flightStructured
  const ob = fs?.outbound
  const ib = fs?.inbound
  logLottetourBasic('register-final', {
    rawTextLen,
    ...
    godId: parsed.godId?.trim() || null,
    evtCd: parsed.evtCd?.trim() || null,
  ...
  if (fail.length) {
    logLottetourBasic('register-final-flags', { failCodes: fail })
  }
}
