/**
 * 롯데관광(lottetour) 등록 일정 표현 SSOT — routeText(a–g ` - `) · description(동선 1줄 + 분위기 2~3문장).
...
const LOTTETOUR_ROUTE_PLACE_NOISE_RE =
  /^(?:호텔\s*조식|조식\s*후|중식|석식|자유\s*시간|체크\s*인|체크\s*아웃|공항\s*도착|공항\s*출발|출발|도착|이동|탑승|귀국|투숙|미팅|피켓|입국\s*수속|출국\s*수속|포함\s*일정|영문\s*가족관계|가족관계\s*증명서|증명서|면세\s*가능|현지\s*가이드|현지\s*연락처|필수\s*서류|작성\s*및\s*제출|롯데관광\s*단독|^롯데$)|^\d+\s*시간(?:\s*\d+\s*분)?$|(?:가정식|쌀국수|분짜|반쎄오|갑오징어|양식|한식|특식).{0,12}(?:SET|세트)|(?:SET|세트)$|(?:소고기\s*)?쌀국수|분짜|반쎄오|가정식|갑오징어(?:\s*볶음)?|(?:양식|한식|특식|BBQ)\s*SET|에그\s*타르트|광동식|^이태원$|^[★☆◈◎○]|기상\s*악화|결항|대체|불가할|유의|안내|주의|※|→|특전|시차|국가번호|관광\s*시간|쇼핑점|침향|찻집|라텍스/i
