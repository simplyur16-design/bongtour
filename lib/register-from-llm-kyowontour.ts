...
} from '@/lib/register-schedule-extract-kyowontour'
import {
...
import { extractStructuredTourSignals } from './structured-tour-signals-kyowontour'
import type { StructuredOptionalTourRow, StructuredShoppingStopRow } from './structured-tour-signals-kyowontour'
import {
...
} from '@/lib/register-llm-blocks-kyowontour'
import {
...
import { filterOptionalTourRows, optionalTourRowPassesStrictGate, type OptionalTourRowFields } from '@/lib/optional-tour-row-gate-kyowontour'
...
import {
  ...
  type DayHotelPlan,
} from '@/lib/day-hotel-plans-kyowontour'
...
} from '@/lib/review-policy-kyowontour'
...
import { parsePricePromotionFromGeminiJson, type PricePromotionSnapshot } from './price-promotion-kyowontour'
...
const REGISTER_BRAND = 'kyowontour' as const
 ...
 * 본문에 이미 포함된 블록은 중복 합치지 않는다(`register-llm-blocks-kyowontour` omit 규칙과 동일한 ⊂ 판별).
 */
...
function kyowontourBlankSignalsWhenDedicatedPasteEmpty(
...
/** kyowontour 등록 전용: 맨 앞 `[배지]`·공백만 정리(`register-kyowontour-basic` 추출 결과 후처리용). */
...
export async function parseForRegisterLlmKyowontour(
  rawText: string,
  ...
  if (!options?.presetDetailBody) {
    throw new Error(
      'parseForRegisterLlmKyowontour: presetDetailBody가 필요합니다. register-parse-kyowontour 및 전용 /api/travel/parse-and-register-* 만 사용하세요.'
    )
  }
  ...
  if (expectedDaysForSchedule != null && expectedDaysForSchedule >= 1) {
    ...
    const sr = await runScheduleExtractLlm(model, blockB, expectedDaysForSchedule, {
      logLabel: forPreview
        ? 'parseForRegisterLlmKyowontour-schedule-first-preview'
        : 'parseForRegisterLlmKyowontour-schedule-first',
    })
    console.info(
      `[kyowontour][timing] schedule-extract-llm +${Date.now() - tScheduleExtract}ms forPreview=${forPreview} expectedDays=${expectedDaysForSchedule} gotRows=${sr.rows.length}`
    )
  ...
  if (finishReason === 'MAX_TOKENS') {
    console.error('[kyowontour-llm] Gemini finishReason=MAX_TOKENS (출력 상한 도달·JSON 잘림 가능)', {
      forPreview,
  ...
  try {
    if (forPreview) {
      const previewLlm = parseLlmJsonObject<Record<string, unknown>>(text, {
        logLabel: 'parseForRegisterLlmKyowontour-preview-minimal',
      })
      ...
      raw = parseLlmJsonObject<RegisterGeminiLlmJson>(text, { logLabel: 'parseForRegisterLlmKyowontour' })
    }
  ...
  if (raw === undefined) {
    ...
    console.error('[kyowontour-llm] LLM JSON parse failed; attempting one repair generateContent call', {
      finishReason,
      length: text.length,
      firstError: firstParseError,
    })
...
${text.slice(0, 16000)}`
    ...
    try {
      if (forPreview) {
        const previewLlm = parseLlmJsonObject<Record<string, unknown>>(text, {
          logLabel: 'parseForRegisterLlmKyowontour-preview-minimal-repair',
        })
        const det = buildPreviewDeterministicRegisterRaw({
          detailBody,
          blockB,
        ...
        raw = parseLlmJsonObject<RegisterGeminiLlmJson>(text, { logLabel: 'parseForRegisterLlmKyowontour-repair-retry' })
      }
  ...
  kyowontourClearLlmWhenDedicatedPasteEmpty(raw, pb)
  ...
  const supplierListingTitleRaw = extractKyowontourVerbatimListingTitle(pasteForTitle)
  ...
  const optionalToursLineRegex = pb.optionalTour?.trim()
    ...
    : null
  const signals = kyowontourBlankSignalsWhenDedicatedPasteEmpty(extractStructuredTourSignals(signalsHaystack), pb)
  ...
  return {
    ...
    kyowontourScheduleExtractFilled,
    ...(forPreview
      ? {
