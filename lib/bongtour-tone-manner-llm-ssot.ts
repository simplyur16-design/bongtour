...
export function buildScheduleExtractToneBlock(): string {
  return (
    ...
    `- **첫 줄만** 잘라 쓰거나 한두 문장으로 끝내지 말 것. 다만 description은 **3~4문장·400자 이내**를 지킨다(정보가 매우 적을 때만 그 이하). 장황한 나열·원문 복붙 금지.\n` +
...
- 일차별로 그날 표 **핵심 흐름**이 드러나게 반영한다. **원문 첫 줄만 잘라 붙이거나 한두 문장으로 끝내는 것은 금지**한다. 일차당 description은 **3~4문장·400자 이내**를 목표로 한다(원문이 짧으면 그에 맞춤).
