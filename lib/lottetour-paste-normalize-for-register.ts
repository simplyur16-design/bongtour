/**
 * 롯데관광(lottetour) 관리자 붙여넣기 — 일정 분할·필드 추출용 전처리.
 */
...
export function normalizeLottetourRegisterPasteLineEndings(raw: string): string {
  return raw.replace(/\r\n/g, '\n').replace(/\r/g, '\n')
}

/** 일정 추출용: UI 줄 제거·연속 공백 축소(탭→공백은 detail-body 쪽과 별도). */
export function normalizeLottetourPasteForScheduleExtract(raw: string): string {
  const lines = normalizeLottetourRegisterPasteLineEndings(raw).split('\n')
  const out: string[] = []
...
export function sliceLottetourItineraryBodyForDayMarkers(raw: string): string {
  const full = normalizeLottetourRegisterPasteLineEndings(raw)
