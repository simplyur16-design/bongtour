...
import {
  enrichLottetourDepartureInputsForConfirmSave,
  loadLottetourProductInfantFallback,
...
} from '@/lib/upsert-itinerary-days-lottetour'
...
import { mergeDayHotelPlansForRegister } from '@/lib/day-hotel-plans-lottetour'
...
import { resolveLottetourHeroTripDates } from '@/lib/product-hero-lottetour'
...
} from '@/lib/flight-manual-correction-lottetour'
import { buildRegisterPreviewCanonicalString } from '@/lib/register-preview-content-fingerprint-lottetour'
import {
  REGISTER_ADMIN_SNAPSHOT_STATUS,
  type RegisterAdminSnapshotStatus,
} from '@/lib/register-admin-audit-status-lottetour'
import { nextRegisterAnalysisAttemptNo } from '@/lib/register-admin-analysis-store-lottetour'
import {
...
import { lottetourBuildMinimalDepartureInputs } from '@/lib/lottetour-synthetic-departure'

type HeroTripDatesSupplement = Partial<Pick<Parameters<typeof resolveLottetourHeroTripDates>[0], 'lottetourFlightStructured'>>
...
/**
 * 롯데관광 전용: 단계별 경과 시간을 항상 서버 로그에 남김(개발·운영 공통).
 * `parse-and-register-timing`은 dev에서만 출력하므로, 4분 지연 구간 특정용으로 별도 사용.
 */
function createLottetourRegisterTiming(logPrefix: string) {
  const t0 = Date.now()
  const devPrefix = `${logPrefix}[timing]`
  function mark(label: string) {
    if (process.env.LOTTETOUR_REGISTER_TIMING_LOG !== '1') return
    const elapsed = Date.now() - t0
    console.info(`[lottetour][timing] ${label} +${elapsed}ms`)
    if (isDev) {
...
export type ParseAndRegisterFlowOptions = {
  /** 라우트 SSOT — 본 파일은 항상 `'lottetour'`만 허용, body.brandKey는 검증용(선택). */
  ...
  /** `resolveLottetourHeroTripDates`에 합쳐 넣을 추가 인자(구조화 항공 blob 등). */
...
function computePreviewContentDigestForBody(body: Record<string, unknown>): string {
  ...
  const brandKey = 'lottetour' as const
...
export async function runLottetourRegisterFlow(request: Request, flowOptions: ParseAndRegisterFlowOptions) {
  ...
  try {
    ...
    const timing = createLottetourRegisterTiming(currentLogPrefix)
    timing.mark('start')
    ...
    assertRegisterRouteSupplierMatch('lottetour', body.originSource, {
    ...
    if (forcedBrandKey !== 'lottetour') {
      return NextResponse.json({ error: '내부 설정 오류: lottetour 전용 오케스트레이션입니다.' }, { status: 500 })
    }
    if (
      brandKeyFromBody &&
      normalizeBrandKeyToCanonicalSupplierKey(brandKeyFromBody) !== 'lottetour'
    ) {
      return NextResponse.json(
        {
          error: '요청 brandKey와 엔드포인트가 맞지 않습니다. 이 API에는 brandKey "lottetour"만 허용됩니다.',
        },
        { status: 400 }
      )
    }
    const brandKey = 'lottetour'
    ...
    if (!text && !hasParsed && !originUrl) {
      return NextResponse.json(
        {
          error:
            '롯데관광 등록에는 공급사 상품 URL 또는 상세 본문 붙여넣기가 필요합니다.',
        },
        { status: 400 }
      )
    }
    ...
    if (patchParsedAfterAugment) {
      ...
      if (
        shouldSkipConfirmDetailPatch({
          ...
          detailCollectRan: parsed.lottetourDetailCollectRan,
    ...
    const lottetourAnchors = extractLottetourTripAnchorsFromPaste(
      text,
      parsed.detailBodyStructured?.flightStructured ?? null
    )
    if (lottetourAnchors.godId && !String(parsed.godId ?? '').trim()) {
      parsed = { ...parsed, godId: lottetourAnchors.godId }
    }
    if (lottetourAnchors.evtCd && !String(parsed.evtCd ?? '').trim()) {
      parsed = { ...parsed, evtCd: lottetourAnchors.evtCd }
    }
    if (lottetourAnchors.categoryMenuNo && !parsed.categoryMenuNo) {
      parsed = { ...parsed, categoryMenuNo: lottetourAnchors.categoryMenuNo }
    }
    if (departureInputs.length === 0 && lottetourAnchors.tripStartIso) {
    ...
    if (isDev && process.env.LOTTETOUR_REGISTER_PERF_LOG === '1') {
      console.info('[register-lottetour]', {
        ...
        tripStartSource: lottetourAnchors.tripStartSource,
        tripEndSource: lottetourAnchors.tripEndSource,
    ...
    const heroResolvedPreview = resolveLottetourHeroTripDates({
      selectedDate: calendarDep,
      fallbackPriceRowDate: calendarDep,
      duration: parsed.duration,
      departureFacts: factsFromRow,
      lottetourFlightStructured: heroTripDatesExtra.lottetourFlightStructured ?? null,
    })
    ...
    const { buildRegisterPreviewSsotMeta } = await import('@/lib/register-preview-ssot-lottetour')
    ...
    if (mode === 'preview') {
      ...
      const { buildRegisterCorrectionPreview } = await import('@/lib/register-correction-preview-lottetour')
    ...
    const heroAuditForMeta = resolveLottetourHeroTripDates({
