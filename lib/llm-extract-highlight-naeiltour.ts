/**
 * 내일투어(naeiltour) 전용 — Gemini로 상품 핵심 포인트 영역 추출·정리.
...
const NAEIL_SYSTEM = `당신은 내일투어 상세 붙여넣기 원문에서 특정 구역만 분리하는 도우미입니다.
...
export async function extractHighlightFromNaeiltourLLM(rawText: string): Promise<{
  highlightPointsRaw: string | null
  highlightPoints: string | null
} | null> {
...
`
  ...
  const model = genAI.getGenerativeModel({
    model: getModelName(),
    systemInstruction: NAEIL_SYSTEM,
  })
  ...
  try {
    ...
    console.warn('[llm-extract-highlight-naeiltour]', e instanceof Error ? e.message : e)
    return null
  }
}
