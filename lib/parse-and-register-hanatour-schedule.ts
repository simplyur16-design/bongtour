...
function _hanatourExtractPlacesInOrder(hay: string): string[] {
  ...
  for (const { name } of hits) {
    ...
    if (_hanatour_MEAL_HOTEL.test(name)) continue
    seen.add(name)
    out.push(name)
  }
  return out
...
function _hanatourFormatRoute(places: string[], hay: string): string {
  let seq = [...places]
  ...
  const firstSeg = hay
    ...
    .find((frag) => {
      ...
      if (_hanatour_MEAL_HOTEL.test(frag)) return false
...
function _hanatourShouldReplaceTitle(title: string, description: string): boolean {
  const t = title.replace(/\s+/g, ' ').trim()
  if (!t || t === '-' || t === '—' || t === '–') return true
  if (_hanatour_DAY_N_TRAVEL.test(t)) return true
  if (_hanatour_BAN_TITLE.test(t)) return true
  if (/^\d{4}[./-]\d{1,2}[./-]\d{1,2}/.test(t) && t.length < 18) return true
  if (t.length > 72) return true
  if (_hanatour_SENTENCE_TAIL.test(t) && t.length > 24) return true
  if (/^[\d\s\-–—·일차제]+$/u.test(t)) return true
  return false
}

function _hanatourDeriveHeaderTitle(day: number, title: string, description: string): string {
  const hay = _hanatourCompactWs(`${title}\n${description}`)
  if (!hay) return ''
  const places = _hanatourExtractPlacesInOrder(hay)
  const route = _hanatourFormatRoute(places, hay)
...
const HANATOUR_CARD_DESCRIPTION_MAX = 400
/** 교체·보강 판정 하한(이 미만이면 규칙 보강·제미나이 후보) */
const HANATOUR_CARD_DESCRIPTION_MIN = 200
/** 설명 밀도 목표 하한(가능하면 이 길이까지 장소·동선으로 채움) */
const HANATOUR_CARD_DESCRIPTION_TARGET = 280
...
/** 출발지·한국 공항 등 — 관광 지명 primaryGeoToken 에서 제외(유럽 상품 본문의 "인천" 오탐 방지) */
function isHanatourAirportOrOriginGeoToken(tok: string): boolean {
  const t = tok.trim()
  if (!t) return true
  if (/^(인천|김포|김해|대구|광주|청주|양양|서울)$/i.test(t)) return true
  if (/^(ICN|GMP|PUS|TAE|CJJ|YNY)$/i.test(t)) return true
  if (/공항/.test(t)) return true
  return false
}
...
function primaryGeoToken(joined: string, title: string): string | null {
  const blob = `${joined}\n${title}`
  const primaryRe =
    /(연길|상해|주가각|백두산|방콕|도쿄|파리|이도백하|장백|금강대|서파|북파|푸동|YNJ|PVG)/gi
  let m: RegExpExecArray | null
  while ((m = primaryRe.exec(blob)) !== null) {
    const tok = m[1]!
    if (!isHanatourAirportOrOriginGeoToken(tok)) return tok
  }
  const titleParts = title.split(/\s*-\s*/).map((s) => s.trim()).filter(Boolean)
  for (const part of titleParts) {
    const runs = part.match(/[\uAC00-\uD7AF]{2,8}/g) ?? []
    for (const g of runs) {
      if (/^(일차|일정|현지)$/.test(g)) continue
      if (isHanatourAirportOrOriginGeoToken(g)) continue
      return g
    }
  }
  const runsTitle = title.match(/[\uAC00-\uD7AF]{2,8}/g) ?? []
  for (const g2 of runsTitle) {
    if (/^(일차|일정|현지)$/.test(g2)) continue
    if (isHanatourAirportOrOriginGeoToken(g2)) continue
    return g2
  }
  return null
}
...
function hanatourTrimOneSentenceMax(s: string, max: number = HANATOUR_CARD_DESCRIPTION_MAX): string {
  ...
  if (t.length > max) {
    ...
    t = (sp > Math.floor(max * 0.25) ? cut.slice(0, sp) : cut).trim()
  }
  if (!/[.!?…]$/.test(t)) t = `${t}.`
...
/**
 * 3~4문장 권장. 200자 미만·유형별 정보 부족 시 코드로 밀도만 보강한다(호텔명·식사 메뉴 없음).
 */
...
/**
 * 일정 카드 description: 한국어 3~4문장·일정 흐름. 호텔·식사·장문 dump 금지. title 과 과도 중복 금지.
 */
...
export function augmentHanatourScheduleExpressionParsed(parsed: RegisterParsed): RegisterParsed {
  ...
  return {
    ...parsed,
    schedule: sched.map((r) => {
      const sanitized = sanitizeHanatourScheduleRowExpression(
        stripCounselingTermsFromScheduleRow(r),
        maxDay,
        productContextBlob
      )
      const title = String(sanitized.title ?? '').trim()
      const description = String(sanitized.description ?? '').trim()
      if (!_hanatourShouldReplaceTitle(title, description)) return sanitized
      const derived = _hanatourDeriveHeaderTitle(sanitized.day, title, description).trim()
      if (!derived) return sanitized
      return { ...sanitized, title: derived.slice(0, 200) }
    }),
  }
}
...
export async function polishHanatourScheduleRowTitleDescriptionWithGeminiIfNeeded(
  ...
  const payload = {
    ...
    code_description_draft: row.description.trim().slice(0, 400),
    ...
    instructions:
      '참고용 hotel/meals는 title·description에 넣지 말 것. JSON만 출력: {"title":"...","description":"..."}. title 12–40자 장소·동선 요약, description 3~4문장·200–400자(최대 약 400자) 마침표로 끝.',
  }
...
description은 3~4문장·200~400자(가능하면 260~380자)로 정보량을 채운다.
