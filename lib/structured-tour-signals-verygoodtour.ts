/**
 * 참좋은여행(verygoodtour) 전용 — structured tour signals (선택관광·쇼핑 등 본문 신호 추출).
 *
 * Gate 연결
 * - 선택관광 행 필터/금칙은 `@/lib/optional-tour-row-gate-verygoodtour`만 import한다. 공용 `optional-tour-row-gate.ts`는 없다.
...
import { filterOptionalTourRows, isBannedOptionalTourName } from '@/lib/optional-tour-row-gate-verygoodtour'
