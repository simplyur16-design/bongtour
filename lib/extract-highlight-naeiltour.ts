/**
 * 내일투어(naeiltour) 전용 — 「Point 상품포인트」 ★ 리스트. 타 공급사와 공유하지 않음.
 */
...
function isNaeiltourStarLine(line: string): boolean {
  const t = line.trim()
  return /^[\s★☆❋✦✧]*★/.test(t) || /^[\s\*]+\S/.test(t)
}

function normalizeNaeiltourLine(line: string): string {
  let t = line.trim()
  t = t.replace(/^[\s★☆❋✦✧]+\s*/, '')
  t = t.replace(/^\*\s+/, '')
...
export function extractHighlightFromNaeiltour(rawHtml: string | unknown): string | null {
  ...
  const lines = slice
    .split(/\n/)
    .filter(isNaeiltourStarLine)
    .map((l) => normalizeNaeiltourLine(l))
    .filter(Boolean)
  if (lines.length === 0) {
