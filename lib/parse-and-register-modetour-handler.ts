...
function _modetourExtractPlacesInOrder(hay: string): string[] {
  ...
  for (const { name } of hits) {
    ...
    if (_modetour_MEAL_HOTEL.test(name)) continue
    seen.add(name)
    out.push(name)
  }
  return out
...
function _modetourFormatRoute(places: string[], hay: string): string {
  let seq = [...places]
  ...
  const firstSeg = hay
    ...
    .find((frag) => {
      ...
      if (_modetour_MEAL_HOTEL.test(frag)) return false
...
function _modetourShouldReplaceTitle(title: string, description: string): boolean {
  const t = title.replace(/\s+/g, ' ').trim()
  if (!t || t === '-' || t === '—' || t === '–') return true
  if (_modetour_DAY_N_TRAVEL.test(t)) return true
  if (_modetour_BAN_TITLE.test(t)) return true
  if (/^\d{4}[./-]\d{1,2}[./-]\d{1,2}/.test(t) && t.length < 18) return true
  if (t.length > 72) return true
  if (_modetour_SENTENCE_TAIL.test(t) && t.length > 24) return true
  if (/^[\d\s\-–—·일차제]+$/u.test(t)) return true
  return false
}

function _modetourDeriveHeaderTitle(day: number, title: string, description: string): string {
  const hay = _modetourCompactWs(`${title}\n${description}`)
  if (!hay) return ''
  const places = _modetourExtractPlacesInOrder(hay)
  const route = _modetourFormatRoute(places, hay)
...
export async function handleParseAndRegisterModetourRequest(request: Request) {
  ...
  try {
    ...
    const scheduleRaw = parsed.schedule ?? []
    const schedule = scheduleRaw.map((r) => {
      const title = String(r.title ?? '').trim()
      const description = String(r.description ?? '').trim()
      if (!_modetourShouldReplaceTitle(title, description)) return r
      const derived = _modetourDeriveHeaderTitle(r.day, title, description).trim()
      if (!derived) return r
      return { ...r, title: derived.slice(0, 200) }
    })
