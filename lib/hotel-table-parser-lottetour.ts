...
/** `[미정]` 등과 함께 `도시A/도시B/…` 형태의 예정 호텔 후보(최대 4)를 후보 배열로 승격 */
function lottetourExpandUndecidedHotelCandidates(line: string, current: string[]): string[] {
  if (current.length >= 2) return current
  if (!/\[?\s*미정\s*\]?|미정\s*호텔|호텔\s*미정/i.test(line)) return current
  const parts = line
    .split('/')
    .map((x) => x.replace(/\s+/g, ' ').trim())
    .filter(Boolean)
  const cities = parts
    .filter((p) => p.length >= 2 && p.length <= 40 && !/^\[?\s*미정|예정|동급|일차|DAY\b/i.test(p))
    .slice(0, 4)
  return cities.length ? cities : current
}

/** 롯데관광(lottetour) 전용 — 호텔 섹션 표·서술에서 행 복원 */
export function parseHotelSectionGeneric(section: string): HotelStructured {
  ...
  const rows = lines
    ...
    .map((l) => {
      ...
      const baseCandidates = hotelCandidates.length > 0 ? hotelCandidates : [l]
      return {
        dayLabel,
        dateText,
        cityText,
        bookingStatusText,
        hotelNameText: l,
        hotelCandidates: lottetourExpandUndecidedHotelCandidates(l, baseCandidates),
        noteText: '',
      }
    })
    .filter((r) => {
    ...
    const scheduleFallback = lines
      ...
      .map((l) => {
        ...
        return {
          ...
          hotelNameText: l,
          hotelCandidates: lottetourExpandUndecidedHotelCandidates(l, sc.length ? sc : [l]),
  ...
  if (rows.length === 0) {
    for (let i = 0; i < lines.length; i++) {
      ...
      const hotelCandidatesRaw = splitCandidates(merged).filter((x) => /(호텔|리조트|숙소|콘도|동급호텔)/i.test(x))
      const hotelCandidates = lottetourExpandUndecidedHotelCandidates(
        merged,
        hotelCandidatesRaw.length ? hotelCandidatesRaw : [line]
      )
