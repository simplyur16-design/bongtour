/**
 * 롯데관광(lottetour) 등록 전용: detail review 집계 + 쇼핑 Gemini 노이즈 필터.
 */
import type {
...
export function buildDetailReviewPolicyLottetour(args: {
  ...
  const bk = canonicalBrandKeyForRegister('lottetour')
  ...
  const joinedSectionsText = sections.map((s) => s.text).join('\n')
  if (/롯데관광\s*단독\s*전세기/i.test(joinedSectionsText)) {
    info.push('롯데관광 단독 전세기 상품 문구 감지 — 항공·변동 조항을 약관과 교차 검수')
  }
