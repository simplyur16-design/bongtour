/**
 * 내일투어(naeiltour) 경로용 — ProductDeparture MVP: 출발일/가격/상태 동기화 적재.
...
/** 내일투어(naeiltour) 전용: 성인만 갱신 입력으로 덮고, 아동·유아는 이번 요청에 숫자가 있을 때만 갱신·없으면 DB 유지 */
function pickPreservedChildInfantPriceNaeiltour(
...
function pickPreservedInfantPriceNaeiltour(
  incoming: number | null | undefined,
  existing: number | null | undefined
): number | null {
 ...
 */
function resolveNaeiltourChildSlotPrice(
  incoming: number | null | undefined,
  ...
  return pickPreservedChildInfantPriceNaeiltour(incoming, previous)
}

/** 동일 상품의 다른 출발행·ProductPrice에서 유아 단가 후보를 찾는다(신규 출발일 행 보강). */
export async function loadNaeiltourProductInfantFallback(
  prisma: PrismaClient,
  productId: string
): Promise<number | null> {
...
export function enrichNaeiltourDepartureInputsForConfirmSave(
...
export async function upsertProductDepartures(
  ...
  for (const { dep: d, departureDate } of pairs) {
    ...
    const childBedPrice = resolveNaeiltourChildSlotPrice(d.childBedPrice, adultPrice, previous?.childBedPrice)
    const childNoBedPrice = resolveNaeiltourChildSlotPrice(d.childNoBedPrice, adultPrice, previous?.childNoBedPrice)
    let infantPrice = pickPreservedInfantPriceNaeiltour(d.infantPrice, previous?.infantPrice)
