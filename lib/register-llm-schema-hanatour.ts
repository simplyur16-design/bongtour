...
import type { PricePromotionSnapshot } from './price-promotion-hanatour'
...
export type RegisterParsed = {
  ...
  /** 잔여 좌석 — rawMeta.structuredSignals·공개 한 줄 표시용 */
  ...
  /** 내부: gw API 상세카드 자동수집(미리보기 adapterPrefetchRan) */
  hanatourDetailCollectRan?: boolean
  hanatourDetailCollectSummary?: string | null
}

/** API·클라이언트 응답용: 감사·관리자 저장 전용만 제거. DetailCollectRan은 confirm 재수집 스킵용으로 유지.
...
export type RegisterLlmParseOptionsCommon = {
  ...
  /**
   * true면 `runScheduleExtractLlm` 선행 호출 생략(메인 confirm 한 번만).
   * 하나투어 기본은 `register-parse-hanatour`에서 true.
   */
  skipScheduleExtractLlm?: boolean
  /** true면 `supplementMustKnowWithWebSearch` 생략(운영 속도). */
  skipMustKnowWebSupplement?: boolean
