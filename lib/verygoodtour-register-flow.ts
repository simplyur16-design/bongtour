...
/**
 * 참좋은여행(verygoodtour) 등록 preview/confirm HTTP 흐름 — API SSOT (Gemini overlay 없음).
...
/** verygoodtour 등록 save 게이트: 달력 행만이 아니라 표·항공 구조화·일정 초안을 함께 본다. */
...
type HeroTripDatesSupplement = Record<string, never>
...
/** verygoodtour 등록 preview/confirm 전용 오케스트레이션 — `handleParseAndRegisterVerygoodtourRequest`만 연결. */
let currentLogPrefix = '[parse-and-register]'
const isDev = process.env.NODE_ENV === 'development'

/**
 * 참좋은여행 전용: 단계별 경과 시간을 항상 서버 로그에 남김(개발·운영 공통).
 * `parse-and-register-timing`은 dev에서만 출력하므로, 지연 구간 특정용으로 별도 사용.
 */
function createVerygoodtourRegisterTiming(logPrefix: string) {
  const t0 = Date.now()
  const devPrefix = `${logPrefix}[timing]`
  function mark(label: string) {
    if (process.env.VERYGOODTOUR_REGISTER_TIMING_LOG !== '1') return
    const elapsed = Date.now() - t0
    console.info(`[verygoodtour][timing] ${label} +${elapsed}ms`)
    if (isDev) {
...
export type ParseAndRegisterFlowOptions = {
  /** 라우트 SSOT — 본 파일은 항상 `'verygoodtour'`만 허용, body.brandKey는 검증용(선택). */
  ...
  /**
   * augmentParsed 이후 한 번 더 패치(예: PackageDetail detail-collect). 브랜드 문자열 분기 없음 — 핸들러가 구현을 넘긴다.
   */
  patchParsedAfterAugment?: (
    ...
    ctx?: {
      ...
      pastedBlocks?: Partial<
        Pick<import('@/lib/register-llm-blocks-verygoodtour').RegisterPastedBlocksInput, 'optionalTour' | 'shopping'>
      > | null
    },
  ...
  /** @deprecated API SSOT 경로에서는 미사용 — 하위 호환용 */
...
function computePreviewContentDigestForBody(body: Record<string, unknown>): string {
  ...
  const brandKey = 'verygoodtour' as const
...
export async function runVerygoodtourRegisterFlow(request: Request, flowOptions: ParseAndRegisterFlowOptions) {
  ...
  try {
    ...
    const timing = createVerygoodtourRegisterTiming(currentLogPrefix)
    timing.mark('start')
    ...
    if (forcedBrandKey !== 'verygoodtour') {
      return NextResponse.json({ error: '내부 설정 오류: verygoodtour 전용 오케스트레이션입니다.' }, { status: 500 })
    }
    if (
      brandKeyFromBody &&
      normalizeBrandKeyToCanonicalSupplierKey(brandKeyFromBody) !== 'verygoodtour'
    ) {
      return NextResponse.json(
        {
          error: '요청 brandKey와 엔드포인트가 맞지 않습니다. 이 API에는 brandKey "verygoodtour"만 허용됩니다.',
        },
        { status: 400 }
      )
    }
    const brandKey = 'verygoodtour'
    ...
    if (!text && !hasParsed && !originUrl) {
      return NextResponse.json(
        {
          error:
            '참좋은여행 등록에는 공급사 상품 URL 또는 상세 본문 붙여넣기가 필요합니다.',
        },
        { status: 400 }
      )
    }
    ...
    if (patchParsedAfterAugment) {
      ...
      if (
        shouldSkipConfirmDetailPatch({
          ...
          detailCollectRan: parsed.verygoodtourDetailCollectRan,
    ...
    if (isDev && process.env.VERYGOODTOUR_REGISTER_PERF_LOG === '1') {
    ...
    if (mode === 'confirm' && strictConfirmDeparturePriceRows === true && departureInputs.length === 0) {
      flowFieldIssues.push({
        field: 'verygoodtourPriceDepartureRows',
        reason:
          '참좋은여행 확정: 출발일별 가격 행(prices[])이 없습니다. HXR·ProCode 달력 수집 후에도 출발일(ISO)이 없으면 합성할 수 없습니다. URL·PriceSeq를 확인하세요.',
        source: 'auto',
      })
    }
    ...
    if (tripAnchors.tripStartIso) {
      heroResolvedPreview = {
        ...heroResolvedPreview,
        departureIso: tripAnchors.tripStartIso,
        departureSource: tripAnchors.tripStartSource,
      }
    }
    if (tripAnchors.tripEndIso) {
      heroResolvedPreview = {
        ...heroResolvedPreview,
        returnIso: tripAnchors.tripEndIso,
        returnSource: tripAnchors.tripEndSource,
      }
    }
    ...
    const verygoodtourFlightLegOk = Boolean(
      fs &&
    ...
    const verygoodtourFlightStructuredOk =
      fsDbg?.status === 'success' || (fsDbg?.status === 'partial' && verygoodtourFlightLegOk) || verygoodtourFlightLegOk
    if (!flightHasUsableCore && !verygoodtourFlightStructuredOk) {
      heroDateFieldIssues.push({
        field: 'flight_info',
    ...
    let heroAuditForMeta = resolveHeroTripDates({
      originSource: effectiveOriginSource,
    ...
    const highlightPointsRaw = extractHighlightFromVerygoodtour(text) ?? null
    ...
    const departureInputsForSave = enrichVerygoodDepartureInputsFromProductPriceTable(
      departureInputs,
      parsed.productPriceTable,
    )
