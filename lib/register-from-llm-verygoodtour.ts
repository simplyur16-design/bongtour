...
import {
  buildMustKnowPipeResultFromNoticeBlob,
  extractVerygoodEtaSnippetForMustKnowBlob,
} from '@/lib/must-know-trip-readiness-pipe-verygoodtour'
import { extractStructuredTourSignals } from './structured-tour-signals-verygoodtour'
import type { StructuredOptionalTourRow, StructuredShoppingStopRow } from './structured-tour-signals-verygoodtour'
import {
...
} from '@/lib/register-llm-blocks-verygoodtour'
import {
...
import { filterOptionalTourRows, optionalTourRowPassesStrictGate, type OptionalTourRowFields } from '@/lib/optional-tour-row-gate-verygoodtour'
...
function requireDirectedFlightLineResolver(options?: RegisterLlmParseOptionsCommon): DirectedFlightLineResolver {
  ...
  if (!fn) {
    throw new Error(
      'parseForRegisterLlmVerygoodtour: resolveDirectedFlightLines가 필요합니다. register-parse-verygoodtour 경로만 사용하세요.',
    )
  }
  return fn
}
...
} from '@/lib/review-policy-verygoodtour'
...
import { parsePricePromotionFromGeminiJson, type PricePromotionSnapshot } from './price-promotion-verygoodtour'
 ...
 * 본문에 이미 포함된 블록은 중복 합치지 않는다(`register-llm-blocks-verygoodtour` omit 규칙과 동일한 ⊂ 판별).
 */
...
/** 전용 입력란 비어 있을 때 본문·regex·LLM 해당 축 미사용 — 참좋은여행 이 파일 전용 */
function verygoodtourClearLlmWhenDedicatedPasteEmpty(
...
function verygoodtourBlankSignalsWhenDedicatedPasteEmpty(
...
- destination 은 **여행여정**·요약의 권역·도시를 우선한다. 숙박·특전·VIP·#태그 한 줄을 destination 으로 쓰지 말 것.
...
# [schedule] 일차별 (필수) — 참좋은여행(verygoodtour) 일정표
...
/** 미리보기: 서술형·장문 키 제거·표 고정(LLM이 넘겨도 무시). 일정 `schedule[]`는 비우지 않음 — preview/save/public detail SSOT가 동일 규칙을 타게 함. */
...
/** 미리보기: 전용 입력란 비어 있으면 LLM·본문 신호로 옵션/쇼핑 메타가 올라오지 않게 고정 */
function applyVerygoodPreviewSsotOptionalShopping(
  raw: RegisterGeminiLlmJson,
  pb: Partial<RegisterPastedBlocksInput>
): RegisterGeminiLlmJson {
  const opt = String(pb.optionalTour ?? '').trim()
  const shop = String(pb.shopping ?? '').trim()
  const rec = raw as Record<string, unknown>
  if (!opt) {
    rec.hasOptionalTour = false
    rec.optionalTourCount = null
    rec.optionalTourSummaryText = null
    delete rec.optionalTourNoticeRaw
    delete rec.optionalTourNoticeItems
    delete rec.optionalTourDisplayNoticeFinal
  }
  if (!shop) {
    rec.hasShopping = false
    rec.shoppingVisitCount = null
    rec.shoppingSummaryText = null
    delete rec.shoppingNoticeRaw
  }
  return raw
}
...
/** 본문 `출국`/`입국` 구조화 결과 → 대표 출발·귀국 일시 원문(관리자/상세 표시용, LLM 없이 결정적) */
function buildVerygoodDepartureArrivalDisplayRawFromFlight(
  fs: DetailBodyParseSnapshot['flightStructured']
): { departure: string | null; arrival: string | null } | null {
  ...
  const fmt = (
    ...
    const apx = (ap ?? '').trim()
    if (!dd && !tt && !apx) return ''
    return `${dd}${dd && tt ? ' ' : ''}${tt}${(tt || dd) && apx ? ' ' : ''}${apx} ${kind}`.trim()
  }
  const legLine = (leg: typeof ob) => {
    const p1 = fmt(leg.departureDate, leg.departureTime, leg.departureAirport, '출발')
    const p2 = fmt(leg.arrivalDate, leg.arrivalTime, leg.arrivalAirport, '도착')
    return [p1, p2].filter(Boolean).join(' → ')
  }
  const dep = legLine(ob)
  const arr = legLine(ib)
...
function buildPreviewDeterministicRegisterRaw(args: {
  ...
  const vgDepArr = buildVerygoodDepartureArrivalDisplayRawFromFlight(detailBody.flightStructured)
  if (vgDepArr?.departure) out.departureDateTimeRaw = vgDepArr.departure
  if (vgDepArr?.arrival) out.arrivalDateTimeRaw = vgDepArr.arrival

  return out
}

export async function parseForRegisterLlmVerygoodtour(
  rawText: string,
  ...
  const clippedForLlm = rawText.trim()
    ? clipVerygoodMarketingTailFromPaste(normalizeVerygoodRegisterPasteLineEndings(rawText.trim())).clipped
    : ''
  const blockB = clippedForLlm ? clippedForLlm.slice(0, REGISTER_PASTE_MAX_CHARS) : EMPTY_PASTE_PLACEHOLDER
  ...
  if (forPreview) {
    ...
    console.info('[verygood-llm] preview Gemini response shape', {
  ...
  if (finishReason === 'MAX_TOKENS') {
    console.error('[verygood-llm] Gemini finishReason=MAX_TOKENS (출력 상한 도달·JSON 잘림 가능)', {
      forPreview,
  ...
  try {
    if (forPreview) {
      const previewLlm = parseLlmJsonObject<Record<string, unknown>>(text, {
        logLabel: 'parseForRegisterLlmVerygoodtour-preview-minimal',
      })
      ...
      raw = applyVerygoodPreviewSsotOptionalShopping(
        finalizePreviewRegisterRaw(mergePreviewDeterministicWithLlm(det, previewLlm)),
        pb
      )
    } else {
      raw = parseLlmJsonObject<RegisterGeminiLlmJson>(text, { logLabel: 'parseForRegisterLlmVerygoodtour' })
    }
    ...
    if (!shouldAttemptJsonRepair) {
      console.error('[verygood-llm] LLM JSON parse failed; repair skipped (truncation/형식 정책)', {
        finishReason,
        length: text.length,
        endsWithClosingBrace: te.endsWith('}'),
    ...
    console.error('[verygood-llm] LLM JSON parse failed; attempting one repair generateContent call', {
      finishReason,
      length: text.length,
      firstError: firstParseError,
    })
...
${text.slice(0, 16000)}`
    ...
    try {
      if (forPreview) {
        const previewLlm = parseLlmJsonObject<Record<string, unknown>>(text, {
          logLabel: 'parseForRegisterLlmVerygoodtour-preview-minimal-repair',
        })
        const det = buildPreviewDeterministicRegisterRaw({
          detailBody,
          blockB,
        ...
        raw = parseLlmJsonObject<RegisterGeminiLlmJson>(text, { logLabel: 'parseForRegisterLlmVerygoodtour-repair-retry' })
      }
  ...
  let scheduleBase: RegisterScheduleDay[] = (raw.schedule ?? [])
    .map((s) => {
      const rec = s as Record<string, unknown>
      return {
        day: Number(s?.day) || 0,
        dateText: strOrNull(rec.dateText),
  ...
  if (schedule.length === 0 && blockB && blockB !== EMPTY_PASTE_PLACEHOLDER && detRows.length > 0) {
    ...
    const itineraryDayDraftsLength = registerScheduleToDayInputs(scheduleFromDet).length
    console.info('[verygoodtour-schedule-blocks]', {
      ...(detExtract?.log ?? {}),
      itineraryDayDraftsLength,
      source: 'deterministic-n일차-blocks',
    })
  ...
  const destResolved = resolveVerygoodtourRegisterDestination({
    pastedBody: pastedBlobForTitle,
    title: titleTrimmed,
    llmDestination,
    bracketDestination: fromBracketDest,
  })
  const finalDestination =
    destResolved.destination ||
    extractNonPolicyDestinationFragment(fromBracketDest ?? '') ||
  ...
  const incExcBlobForMustKnow = detailBody.sections
    .filter((s) => s.type === 'included_excluded_section')
    .map((s) => s.text)
    .join('\n\n')

  const buildVerygoodMustKnowMergedBlob = () => {
    const noticeBlob = detailBody.sections
      .filter((s) => s.type === 'notice_section')
      .map((s) => s.text)
      .join('\n\n')
    const fromPaste = (options?.pastedBlocks?.requiredChecks ?? '').trim()
    const etaSnippet = extractVerygoodEtaSnippetForMustKnowBlob(incExcBlobForMustKnow)
    return [noticeBlob, fromPaste, etaSnippet].filter(Boolean).join('\n\n')
  }

  if (forPreview && mustKnowItems.length === 0) {
    const merged = buildVerygoodMustKnowMergedBlob()
    if (merged.trim()) {
      const pipe = buildMustKnowPipeResultFromNoticeBlob(merged)
      if (pipe.mustKnowItems?.length) {
    ...
    forPreview &&
    mustKnowItems.some(
      (x) => !String(x.body ?? '').trim() && /영국\s*ETA|ETA\s*비용/i.test(String(x.title ?? ''))
    )
  ) {
    const merged = buildVerygoodMustKnowMergedBlob()
    if (merged.trim()) {
      const pipe = buildMustKnowPipeResultFromNoticeBlob(merged)
      const etaFix = pipe.mustKnowItems?.find(
        (x) => /영국\s*ETA|ETA\s*비용/i.test(x.title) && String(x.body ?? '').trim().length > 0
      )
      if (etaFix?.body) {
        mustKnowItems = mustKnowItems.map((x) =>
          !String(x.body ?? '').trim() && /영국\s*ETA|ETA\s*비용/i.test(String(x.title ?? ''))
            ? { ...x, body: etaFix.body }
            : x
        )
      }
  ...
  const segAirline = segmentSupplierPasteForLlm(blockB !== EMPTY_PASTE_PLACEHOLDER ? blockB : '').airlineMeeting
  const flightSources = [
    rawText.trim(),
  ...
  const optionalToursLineRegex = pb.optionalTour?.trim()
    ...
    : null
  const signals = verygoodtourBlankSignalsWhenDedicatedPasteEmpty(
    extractStructuredTourSignals(signalsHaystack),
    pb
  )
  ...
  const shoppingPaste = String(pb.shopping ?? '').trim()
  ...
  const optionalTourNoticeItemsFinal =
    optionalPaste
      ? noticeItemsLlm.length > 0
        ? noticeItemsLlm
        : signals.optionalTourNoticeItems ?? []
      : []

  let optCountFinal = optionalPaste ? optionalRowsDetailCount : 0
  if (optionalPaste && optCountFinal === 0 && optionalToursStructuredFinal) {
    try {
  ...
  const hasOptionalTourFinal = Boolean(
    optCountFinal > 0 ||
      (optionalPaste &&
        ((manualPasteAxes.hasManualOptionalInput && Boolean(optionalPaste)) ||
          optionalTourNoticeItemsFinal.length > 0 ||
          Boolean(raw.hasOptionalTour) ||
          Boolean(signals.hasOptionalTour)))
  )
  ...
  let shopRows = shoppingPaste
    ? shopRowsDetailCount > 0
      ? shopRowsDetailCount
      : shopCountFromJson()
    : 0
  const hasShoppingFinal = Boolean(
    shopRows > 0 ||
      (shoppingPaste &&
        (Boolean(raw.hasShopping) ||
          Boolean(signals.hasShopping) ||
          (manualPasteAxes.hasManualShoppingInput && Boolean(shoppingPaste)) ||
  ...
  const shoppingVisitCountFinal =
    manualPasteAxes.hasManualShoppingInput && Boolean(shoppingPaste) && shopRows <= 0
      ? null
      : shoppingPaste
  ...
  const verygoodDepArrRaw = buildVerygoodDepartureArrivalDisplayRawFromFlight(detailBody.flightStructured)
  ...
  if (!shoppingPaste) {
    shoppingStopsLlmSupplementJson = null
  }
  ...
  return {
    ...
    optionalTourCount:
      optionalTourCountFinal == null ? undefined : optionalTourCountFinal,
    optionalTourSummaryText: optionalTourSummaryFinal,
    shoppingNoticeRaw:
      shoppingPaste
        ? (raw.shoppingNoticeRaw ?? '').trim() || signals.shoppingNoticeRaw || null
        : null,
    shoppingStops:
      shoppingPaste
    ...
    departureDateTimeRaw: manualPasteAxes.hasManualFlightInput
      ? null
      : strOrNull(raw.departureDateTimeRaw) ?? verygoodDepArrRaw?.departure ?? null,
    arrivalDateTimeRaw: manualPasteAxes.hasManualFlightInput
      ? null
      : strOrNull(raw.arrivalDateTimeRaw) ?? verygoodDepArrRaw?.arrival ?? null,
