...
} from '@/lib/register-admin-input-digest-ybtour'
import {
...
/** ybtour 등록 preview/confirm 전용 — `handleParseAndRegisterYbtourRequest`만 연결. */
let currentLogPrefix = '[parse-and-register]'
const isDev = process.env.NODE_ENV === 'development'

/**
 * 노랑풍선 전용: 단계별 경과 시간을 항상 서버 로그에 남김(개발·운영 공통).
...
export type ParseAndRegisterFlowOptions = {
  /** 라우트 SSOT — 본 파일은 항상 `'ybtour'`만 허용, body.brandKey는 검증용(선택). */
  ...
  /**
   * augmentParsed 이후 한 번 더 패치(예: ybtour papi 상세카드). 브랜드 문자열 분기 없음 — 핸들러가 구현을 넘긴다.
   */
  ...
  /** `resolveYbtourHeroTripDates`에 합쳐 넣을 추가 인자(구조화 항공 blob 등). */
...
export async function runYbtourRegisterFlow(request: Request, flowOptions: ParseAndRegisterFlowOptions) {
  ...
  try {
    ...
    if (!text && !hasParsed && !originUrl) {
      return NextResponse.json(
        {
          error:
            '노랑풍선 등록에는 공급사 상품 URL 또는 상세 본문 붙여넣기가 필요합니다.',
        },
        { status: 400 }
      )
    }
    ...
    if (patchParsedAfterAugment) {
      ...
      if (
        shouldSkipConfirmDetailPatch({
          ...
          detailCollectRan: parsed.ybtourDetailCollectRan,
    ...
    const autoExtracted = {
      ...
      originUrl,
      adapterPrefetchRan: Boolean(parsed.ybtourDetailCollectRan),
      ...
      adapterSummaryPreview:
        parsed.ybtourDetailCollectSummary ??
        (parsed.ybtourDetailCollectRan === false
