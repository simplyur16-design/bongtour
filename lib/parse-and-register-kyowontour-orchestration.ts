...
export type ParseAndRegisterFlowOptions = {
  ...
  /**
   * augmentParsed 이후 한 번 더 패치(예: 교원이지 선택관광/쇼핑 AJAX). 브랜드 문자열 분기 없음 — 핸들러가 구현을 넘긴다.
   */
  ...
  try {
    ...
    const autoExtracted = {
      ...
      adapterPrefetchRan: Boolean(
        parsed.kyowontourOptShopCollectRan ||
          parsed.kyowontourScheduleCollectRan ||
          parsed.kyowontourCoreCollectRan,
      ),
      ...
      adapterSummaryPreview:
        [
          ...
          .filter(Boolean)
          .join(' · ') ||
        (parsed.kyowontourOptShopCollectRan === false ||
        parsed.kyowontourScheduleCollectRan === false ||
        parsed.kyowontourCoreCollectRan === false
