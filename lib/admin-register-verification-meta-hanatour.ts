/** [hanatour] admin-register-verification-meta */
/**
    ...
    try {
      ...
      if (sig && typeof sig === 'object' && !Array.isArray(sig)) {
        storedStructuredSignalsPreview = {
          ...
          remainingSeatsCount: sig.remainingSeatsCount,
          departureStatusText: sig.departureStatusText,
          meetingInfoRaw: sig.meetingInfoRaw,
          meetingPlaceRaw: sig.meetingPlaceRaw,
          meetingFallbackText: sig.meetingFallbackText,
        }
      }
    } catch {
      ...
      optionalRows:
        optRows.length > 0
          ? optRows.slice(0, 12).map((r) => ({
              tourName: (r as { tourName?: string; name?: string }).tourName ?? (r as { name?: string }).name ?? '',
