...
// REGRESSION-FREEZE[register-pre-photo-heal-pending-fail2]: 미동부 제목 dest — manifest
const DEST_FROM_TITLE_RE =
  /울란바토르|몽골|도쿄|동경|오사카|다낭|푸꾸옥|나트랑|달랏|연태|하와이|파리|런던|후쿠오카|오키나와|사이판|발리|홍콩|마카오|세부|보라카이|이집트|영국|스위스|이태리|이탈리아|스페인|포르투갈|폴란드|괌|중남미|(?<![가-힣])남미|미동부|미서부|캐나다|시드니|코카서스|튀니지|서안|호이안|바나|위해|서부지중해|동부지중해|지중해|알래스카|캐리비안|토스카나|보르도|두바이|아부다비|고치|나가노|도야마|발틱|장가계|원가계|프랑스|북해도|홋카이도|아이슬란드|그리스|튀르키예|터키|에게/
...
export function inferRegisterPendingDestinationFromTitle(title: string): string {
  ...
  if (m[0] === '미동부') return '미동부'
  if (m[0] === '미서부') return '미서부'
  return m[0]
}
