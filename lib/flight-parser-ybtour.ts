...
/**
 * 노랑풍선(ybtour) 관리자 항공 붙여넣기 전용 — `출발`/`도착` 블록·편명·공항·일시.
 * (공용 `parseFlightSectionGeneric` 미사용)
 */
export function parseFlightSectionYbtour(
  section: string,
  ...
  let blocks =
    full.length >= 32 ? parseYbtourTravelMainScheduleFlight(full) : null
  if (!blocks?.outbound?.flightNo || !blocks?.inbound?.flightNo) {
    blocks = full.length >= 32 ? parseYbtourTravelMainScheduleFlightFromTable(full) : null
  }
  if (!blocks?.outbound?.flightNo || !blocks?.inbound?.flightNo) {
  ...
  if (!blocks?.outbound?.flightNo || !blocks?.inbound?.flightNo) {
    const reviewReasons = ['노랑풍선 출발/도착 블록 또는 편명·일시 추출 실패']
    return {
  ...
  if (status !== 'success') reviewReasons.push('노랑풍선 항공 구조화 일부만 성공 — 원문 검수')

  const selectedOutRaw = ybtourSynthesizePreferredRaw(blocks.outbound, '가는편')
  const selectedInRaw = ybtourSynthesizePreferredRaw(blocks.inbound, '오는편')
