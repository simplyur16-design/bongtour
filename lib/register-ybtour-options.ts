...
function collectYbtourFeeAnchors(lines: string[]): YbFeeAnchor[] {
...
function collectYbtourEuroFeeAnchors(lines: string[]): YbFeeAnchor[] {
  const raw: YbFeeAnchor[] = []
  for (let i = 0; i < lines.length; i++) {
    const L = lines[i]!
    const inline =
    ...
    if (inline) {
      raw.push({ feeLineIdx: i, priceText: `${inline[1]}유로`, currency: 'EUR' })
      continue
    }
    if (/^비용$/i.test(L)) {
      const n = lines[i + 1] ?? ''
      const m = n.match(/^[:：]?\s*(\d+)\s*유로/i) || n.match(/^(\d+)\s*EUR/i)
      if (m) raw.push({ feeLineIdx: i, priceText: `${m[1]}유로`, currency: 'EUR' })
    }
  }
  return dedupeYbtourFeeAnchorsSamePriceWithoutRefBetween(raw, lines)
}
...
function parseYbtourEuroMetaSlice(slice: string[]): { durationText: string; descriptionText: string } {
  let durationText = ''
  let i = 0
  while (i < slice.length) {
    ...
    if (/^참고사항$/i.test(L)) {
      ...
      return { durationText, descriptionText: rest.slice(0, 8000) }
    }
    i++
  }
  return { durationText, descriptionText: '' }
}

function parseYbtourEuroWebsiteRows(lines: string[], anchors: YbFeeAnchor[]): OptionalToursStructured['rows'] {
  ...
  for (let a = 0; a < anchors.length; a++) {
    ...
    const meta = parseYbtourEuroMetaSlice(metaSlice)
...
function parseYbtourEuroWebsiteSection(section: string): OptionalToursStructured {
  const normalized = normalizeYbtourOptionalPasteSection(section)
  const lines = normalized.split('\n').map((l) => l.replace(/\s+/g, ' ').trim())
  const anchors = collectYbtourEuroFeeAnchors(lines)
  if (anchors.length === 0) {
    return { rows: [], reviewNeeded: false, reviewReasons: [] }
  }
  const rows = parseYbtourEuroWebsiteRows(lines, anchors)
  return { rows, reviewNeeded: false, reviewReasons: [] }
}
...
function parseYbtourBlock(lines: string[], start: number, end: number): {
  durationText: string
...
/** 노랑풍선 관리자 옵션 칸 붙여넣기 — 유로 웹본문 우선, 없으면 `$` 블록. */
export function parseYbtourOptionalTourPasteSection(section: string): OptionalToursStructured {
  const euroFirst = parseYbtourEuroWebsiteSection(section)
  if (euroFirst.rows.length > 0) return euroFirst

  const raw = normalizeYbtourOptionalPasteSection(section).replace(/\r/g, '\n')
  const lines = raw.split('\n').map((l) => l.replace(/\s+/g, ' ').trim())
  const anchors = collectYbtourFeeAnchors(lines)
  if (anchors.length === 0) {
    return { rows: [], reviewNeeded: false, reviewReasons: [] }
  }
  ...
  for (let a = 0; a < anchors.length; a++) {
    ...
    const meta = parseYbtourBlock(lines, feeLineIdx, nextStart)
...
export function filterYbtourOptionalTourRows(
...
export function ybtourOptionalPasteDominatesUnstructured(section: string, pasteRowCount: number): boolean {
