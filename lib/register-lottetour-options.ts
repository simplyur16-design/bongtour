...
import { splitPasteTableLineCells } from '@/lib/structured-tour-signals-lottetour'
...
type LottetourFeeAnchor = { feeLineIdx: number; priceText: string; currency: string }

/** 옵션 칸 전용: UI 잔재 제거(개행 유지). */
export function normalizeLottetourOptionalPasteSection(section: string): string {
...
function collectLottetourFeeAnchors(lines: string[]): LottetourFeeAnchor[] {
  const raw: LottetourFeeAnchor[] = []
  ...
  return dedupeLottetourFeeAnchorsSamePriceWithoutRefBetween(raw, lines)
}
 ...
 */
function dedupeLottetourFeeAnchorsSamePriceWithoutRefBetween(anchors: LottetourFeeAnchor[], lines: string[]): LottetourFeeAnchor[] {
  const out: LottetourFeeAnchor[] = []
...
function lottetourHasDetailEuroFeeBlock(lines: string[], fromIdx: number): boolean {
...
function summarizeLottetourOptionalRefWaiting(noteBlob: string): string {
...
function lottetourEuroPriceDisplay(amount: string): string {
...
function collectLottetourEuroFeeAnchors(lines: string[]): LottetourFeeAnchor[] {
  const raw: LottetourFeeAnchor[] = []
  for (let i = 0; i < lines.length; i++) {
    const L = lines[i]!
    const euroSignInline = L.match(/^비용\s*€\s*(\d+)/i)
    if (euroSignInline) {
      if (!lottetourHasDetailEuroFeeBlock(lines, i)) {
        raw.push({ feeLineIdx: i, priceText: lottetourEuroPriceDisplay(euroSignInline[1]!), currency: 'EUR' })
      }
      continue
    }
    const inline =
    ...
    if (inline) {
      raw.push({ feeLineIdx: i, priceText: lottetourEuroPriceDisplay(inline[1]!), currency: 'EUR' })
      continue
    }
    if (/^비용$/i.test(L)) {
      const n = lines[i + 1] ?? ''
      ...
      if (mEuro) {
        raw.push({ feeLineIdx: i, priceText: lottetourEuroPriceDisplay(mEuro[1]!), currency: 'EUR' })
        continue
      }
      const m = n.match(/^[:：]?\s*(\d+)\s*유로/i) || n.match(/^(\d+)\s*EUR/i)
      if (m) raw.push({ feeLineIdx: i, priceText: lottetourEuroPriceDisplay(m[1]!), currency: 'EUR' })
    }
  }
  return dedupeLottetourFeeAnchorsSamePriceWithoutRefBetween(raw, lines)
}
...
function lottetourTitleForEuroAnchor(lines: string[], feeLineIdx: number): string {
...
function parseLottetourEuroMetaSlice(slice: string[]): {
  durationText: string
  ...
  while (i < slice.length) {
    ...
    if (/^참고사항$/i.test(L)) {
      ...
      return {
        ...
        waitingPlaceText: summarizeLottetourOptionalRefWaiting(body),
      }
    }
    i++
  }
...
function parseLottetourEuroWebsiteRows(lines: string[], anchors: LottetourFeeAnchor[]): OptionalToursStructured['rows'] {
  ...
  for (let a = 0; a < anchors.length; a++) {
    ...
    const meta = parseLottetourEuroMetaSlice(metaSlice)
    const title = lottetourTitleForEuroAnchor(lines, feeLineIdx)
    rows.push({
      tourName: title,
      currency,
...
export function parseLottetourOptionalTourPasteTab(raw: string): OptionalToursStructured | null {
...
function parseLottetourEuroWebsiteSection(section: string): OptionalToursStructured {
  const normalized = normalizeLottetourOptionalPasteSection(section)
  const lines = normalized.split('\n').map((l) => l.replace(/\s+/g, ' ').trim())
  const anchors = collectLottetourEuroFeeAnchors(lines)
  if (anchors.length === 0) {
    return { rows: [], reviewNeeded: false, reviewReasons: [] }
  }
  const rows = parseLottetourEuroWebsiteRows(lines, anchors)
  return { rows, reviewNeeded: false, reviewReasons: [] }
}

function lottetourTitleForAnchor(lines: string[], anchorIdx: number): string {
...
function parseLottetourBlock(lines: string[], start: number, end: number): {
  durationText: string
...
export function parseLottetourOptionalTourPasteSection(section: string): OptionalToursStructured {
  const tabbed = parseLottetourOptionalTourPasteTab(section)
  if (tabbed && tabbed.rows.length > 0) return tabbed

  const euroFirst = parseLottetourEuroWebsiteSection(section)
  if (euroFirst.rows.length > 0) return euroFirst

  const raw = normalizeLottetourOptionalPasteSection(section).replace(/\r/g, '\n')
  const lines = raw.split('\n').map((l) => l.replace(/\s+/g, ' ').trim())
  const anchors = collectLottetourFeeAnchors(lines)
  if (anchors.length === 0) {
    return { rows: [], reviewNeeded: false, reviewReasons: [] }
  }
  ...
  for (let a = 0; a < anchors.length; a++) {
    ...
    const title = lottetourTitleForAnchor(lines, feeLineIdx)
    const meta = parseLottetourBlock(lines, feeLineIdx, nextStart)
...
export function filterLottetourOptionalTourRows(
...
export function lottetourOptionalPasteDominatesUnstructured(section: string, pasteRowCount: number): boolean {
