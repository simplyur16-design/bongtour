/**
 * 노랑풍선 등록 가격: 3슬롯(성인·아동·유아). 쿠폰·총액·잔여석·적립·무이자 등 메타는 슬롯 제외.
 */
...
export function extractYbtourThreeSlotPricesFromBlob(blob: string): YbtourThreeSlotExtract | null {
...
export function finalizeYbtourProductPriceTable(
  table: RegisterParsed['productPriceTable'] | null | undefined,
  blob: string
): RegisterParsed['productPriceTable'] | null {
  const fromBlob = extractYbtourThreeSlotPricesFromBlob(blob)
  const t = table ?? null
...
export function finalizeYbtourRegisterParsedPricing(parsed: RegisterParsed): RegisterParsed {
  const blob = ybtourPriceBlobFromParsed(parsed)
  const next = finalizeYbtourProductPriceTable(parsed.productPriceTable ?? null, blob)
