/**
 * 교보이지(kyowontour) 전용 — 선택관광(현지옵션) 행 게이트.
 *
 * 계약
 * - 이 모듈은 교보이지 계열 등록/붙여넣기 본문에서 추출한 선택관광 행만 판별한다. 타 공급사 표를 공용 규칙으로 흡수하지 않는다.
 ...
 *
 * 전용 차이(최소)
 * - `BAN_NAME_REGEX`의 브랜드 마일리지 안내 줄: 교보이지 브랜드 표기를 패턴 앞쪽에 둔다(가시성·추후 분기 여지).
 */
...
const BAN_NAME_REGEX: RegExp[] = [
  ...
  // 브랜드 마일리지 안내 줄: 이 파일은 교보이지 표기를 앞에 둔다(의미 동일, 공급사 전용 파일 가시성).
  /교보이지\s*마일리지|하나투어\s*마일리지|모두투어\s*마일리지|참좋은여행\s*마일리지/i,
]
