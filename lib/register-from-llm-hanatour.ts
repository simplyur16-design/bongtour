...
import { extractStructuredTourSignals } from './structured-tour-signals-hanatour'
import type { StructuredOptionalTourRow, StructuredShoppingStopRow } from './structured-tour-signals-hanatour'
import {
  extractHanatourShoppingVisitCountFromFullBodyText,
  parseHanatourShoppingVisitCountFromText,
...
} from '@/lib/register-llm-blocks-hanatour'
import {
...
import { filterOptionalTourRows, optionalTourRowPassesStrictGate, type OptionalTourRowFields } from '@/lib/optional-tour-row-gate-hanatour'
...
import {
  ...
  type DayHotelPlan,
} from '@/lib/day-hotel-plans-hanatour'
...
const REGISTER_BRAND = 'hanatour' as const
 ...
 * 본문에 이미 포함된 블록은 중복 합치지 않는다(`register-llm-blocks-hanatour` omit 규칙과 동일한 ⊂ 판별).
 */
...
function parseHanatourShoppingVisitCountFromDetailShoppingStructured(
  shoppingStructured: DetailBodyParseSnapshot['shoppingStructured'],
  fullBodyHaystack?: string | null
): number | null {
  if (fullBodyHaystack?.trim()) {
    const fromFull = extractHanatourShoppingVisitCountFromFullBodyText(fullBodyHaystack)
    if (fromFull != null) return fromFull
  }
  return parseHanatourShoppingVisitCountFromText(shoppingStructured.shoppingCountText)
}
...
/** 웹 검색 보강은 지연·비용 큼 — 본문/LLM에서 이미 실질 항목이 있으면 생략 */
function shouldHanatourRunMustKnowWebSupplement(
  items: Array<{ title?: string; body?: string }>
): boolean {
  if (!isMustKnowInsufficient(items)) return false
  const total = items.reduce((n, it) => n + (it.title?.length ?? 0) + (it.body?.length ?? 0), 0)
  if (items.length >= 2 && total >= 160) return false
  if (items.length === 1 && total >= 100) return false
  return true
}
...
/**
 * 하나투어 등록 전용: 상품명 맨 앞 `[배지]`만 제거·공백 정리(LLM 요약·해시 제거 금지).
 * 타 공급사/공용 모듈로 올리지 않는다.
 */
function normalizeHanatourRegisterTitleMinimalLocal(s: string): string {
  let t = s.replace(/\r\n/g, '\n').replace(/\r/g, '\n').trim()
  t = t.replace(/^(\[[^\]\n]{1,120}\]\s*)+/, '')
  t = t.replace(/[\u00a0\u3000]+/g, ' ')
...
/** 붙여넣기 상단부에서 하나투어 상품 리스트 제목 한 줄을 원문 그대로(배지·# 유지) 추출 */
function extractHanatourVerbatimListingTitleRawFromPasteLocal(blob: string): string | null {
...
function llmOptionalToursToStructured(rows: Array<Record<string, unknown>> | undefined): {
  ...
  for (const r of rows) {
    ...
    out.push({
      ...
      alternateScheduleText: strOrNull((r as { alternateScheduleText?: string }).alternateScheduleText),
...
function buildHanatourRegisterHotelSummaryFromNames(names: readonly string[]): string | null {
...
export function resolveHanatourRegisterHotelSummaryFromLlmAndNames(
  existingSummary: unknown,
  hotelNames: readonly string[]
): string | null {
  const fromLlm = strOrNull(existingSummary as string | null | undefined)
  if (fromLlm) return fromLlm
  return buildHanatourRegisterHotelSummaryFromNames(hotelNames)
}

function _hanatourIsPlaceholderHotelListName(n: string): boolean {
...
function _hanatourHasConfirmedHotelFromNames(names: readonly string[]): boolean {
  return names.some((n) => !_hanatourIsPlaceholderHotelListName(n))
}

function _hanatourSummaryLooksLikeSingleHotelNameLine(summary: string | null | undefined): boolean {
...
function _hanatourSummaryLooksLikeConfirmedHotelProduct(
  summary: string | null | undefined,
  names: readonly string[]
): boolean {
  if (_hanatourHasConfirmedHotelFromNames(names)) return true
  const s = String(summary ?? '').replace(/\s+/g, ' ').trim()
  if (!s) return false
  const m = s.match(/^(.+?)\s+외\s+(\d+)\s*개\s*$/)
  if (m?.[1] && !_hanatourIsPlaceholderHotelListName(m[1])) return true
  if (_hanatourSummaryLooksLikeSingleHotelNameLine(s)) return true
  return false
}

function _hanatourIsHotelProductUndeterminedForNotice(summary: string | null, names: readonly string[]): boolean {
  if (_hanatourSummaryLooksLikeConfirmedHotelProduct(summary, names)) return false
  const s = String(summary ?? '').trim()
  if (!s && names.length === 0) return true
  const onlyPlaceholders = names.length === 0 || names.every(_hanatourIsPlaceholderHotelListName)
...
function extractHanatourHotelDepartureNoticeDaysFromHaystack(haystack: string): number {
  const h = haystack.replace(/\s+/g, ' ')
  const m1 = h.match(/출발\s*(\d+)\s*일\s*전까지\s*확정/i)
  if (m1?.[1]) {
    const d = parseInt(m1[1], 10)
    if (d >= 1 && d <= 14) return d
  }
  const m2 = h.match(/(\d+)\s*일\s*전\s*카카오/i)
  if (m2?.[1]) {
    const d = parseInt(m2[1], 10)
    if (d >= 1 && d <= 14) return d
  }
  const m3 = h.match(/출발\s*(\d+)\s*일\s*전까지[^.\n]{0,80}(?:알려|안내)/i)
  if (m3?.[1]) {
    const d = parseInt(m3[1], 10)
    if (d >= 1 && d <= 14) return d
  }
  const m4 = /출발\s*(\d+)\s*일\s*전까지/i.exec(h)
  if (m4?.[1]) {
...
export function applyHanatourHotelUndeterminedDepartureNotice(
  summary: string | null,
  hotelNames: readonly string[],
  haystack: string
): string | null {
  if (!_hanatourIsHotelProductUndeterminedForNotice(summary, hotelNames)) return summary
  const base = (summary?.replace(/\s+/g, ' ').trim() || '미정')
  if (/\(출발\s*\d+\s*일전까지\s*안내\)/.test(base)) return summary
  const days = extractHanatourHotelDepartureNoticeDaysFromHaystack(haystack)
...
function buildPreviewDeterministicRegisterRaw(args: {
  ...
  if (hotelRows.length) {
    ...
    if (names.length) {
      ...
      out.hotelInfoRaw = hotelRows
        ...
        .filter(Boolean)
        .join('\n')
      const summaryLine = buildHanatourRegisterHotelSummaryFromNames(names)
  ...
  const visitFromBody = extractHanatourShoppingVisitCountFromFullBodyText(
    [detailBody.normalizedRaw, blockB, shopCountText].filter(Boolean).join('\n')
  )
  if (shopRowCount > 0 || shopCountText || visitFromBody != null) {
    out.hasShopping = true
    if (visitFromBody != null) out.shoppingVisitCount = visitFromBody
    out.shoppingSummaryText =
      shopCountText ||
      (visitFromBody != null
        ? `쇼핑 ${visitFromBody}회`
        : shopRowCount > 0
          ? `쇼핑 후보 ${shopRowCount}곳`
          : null)
  }
...
export async function parseForRegisterLlmHanatour(
  rawText: string,
  ...
  if (forPreview) {
    ...
    console.info('[hanatour-llm] preview Gemini response shape', {
  ...
  try {
    if (forPreview) {
      const previewLlm = parseLlmJsonObject<Record<string, unknown>>(text, {
        logLabel: 'parseForRegisterLlmHanatour-preview-minimal',
      })
      ...
      raw = parseLlmJsonObject<RegisterGeminiLlmJson>(text, { logLabel: 'parseForRegisterLlmHanatour' })
    }
    ...
    if (!shouldAttemptJsonRepair) {
      console.error('[hanatour-llm] LLM JSON parse failed; repair skipped (truncation/형식 정책)', {
        finishReason,
        length: text.length,
        endsWithClosingBrace: te.endsWith('}'),
    ...
    console.error('[hanatour-llm] LLM JSON parse failed; attempting one repair generateContent call', {
      finishReason,
      length: text.length,
      firstError: firstParseError,
    })
...
${text.slice(0, 16000)}`
    ...
    try {
      if (forPreview) {
        const previewLlm = parseLlmJsonObject<Record<string, unknown>>(text, {
          logLabel: 'parseForRegisterLlmHanatour-preview-minimal-repair',
        })
        const det = buildPreviewDeterministicRegisterRaw({
          detailBody,
          blockB,
        ...
        raw = parseLlmJsonObject<RegisterGeminiLlmJson>(text, { logLabel: 'parseForRegisterLlmHanatour-repair-retry' })
      }
  ...
  if (
    !forPreview &&
    !options?.skipMustKnowWebSupplement &&
    isMustKnowInsufficient(mustKnowItems) &&
    shouldHanatourRunMustKnowWebSupplement(mustKnowItems)
  ) {
  ...
  const visitCountFromDetailBody = parseHanatourShoppingVisitCountFromDetailShoppingStructured(
    detailBody.shoppingStructured,
    signalsHaystack
  )
  ...
  const shoppingVisitCountFinal =
    manualPasteAxes.hasManualShoppingInput && Boolean(String(pb.shopping ?? '').trim()) && shopRows <= 0
      ? null
      : visitCountFromDetailBody != null
        ? visitCountFromDetailBody
  ...
  const rsNums = numericFieldsFromHanatourReservationStatusParsed(detailBody.raw?.hanatourReservationStatus ?? null)
  ...
  const minimumDepartureCount = rsNums?.minimumDepartureCount ?? depExtract.minimumDepartureCount ?? llmMin
  const currentBookedCount = rsNums?.currentBookedCount ?? depExtract.currentBookedCount ?? llmBooked
  const remainingSeatsCountMerged = rsNums?.remainingSeatsCount ?? depExtract.remainingSeatsCount
  ...
  const departureStatusText =
    buildDepartureStatusDisplay({
      ...
      remainingSeatsCount: remainingSeatsCountMerged,
  ...
  const hotelSummaryResolvedHanatour = resolveHanatourRegisterHotelSummaryFromLlmAndNames(
    (raw as { hotelSummaryText?: unknown }).hotelSummaryText,
    hotelNamesForProductSummary
  )
  const hotelSummaryWithNoticeHanatour = applyHanatourHotelUndeterminedDepartureNotice(
    hotelSummaryResolvedHanatour,
    hotelNamesForProductSummary,
  ...
  return {
    ...
    hotelSummaryText: hotelSummaryWithNoticeHanatour,
    hotelStatusText: strOrNull(raw.hotelStatusText),
    hotelNoticeRaw: strOrNull(raw.hotelNoticeRaw),
    ...
    remainingSeatsCount: remainingSeatsCountMerged ?? null,
