...
} from '@/lib/register-admin-input-digest-modetour'
...
export type ModetourRegisterFlowOptions = {
  forcedBrandKey: 'modetour'
  parseFn: typeof parseForRegisterModetour
  logPrefix: string
...
export async function runModetourRegisterFlow(request: Request, flowOptions: ModetourRegisterFlowOptions) {
  const parseFn = flowOptions.parseFn
  const forcedBrandKey = flowOptions.forcedBrandKey
  const { patchParsedAfterAugment, savePersistedParsedOnly } = flowOptions
  ...
  try {
    ...
    if (!text && !hasParsed && !originUrl) {
      return NextResponse.json(
        {
          error:
            '모두투어 등록에는 상품 URL(originUrl)이 필요합니다. [사실 가져오기]로 본문을 채우거나 URL을 입력하세요.',
        },
        { status: 400 }
      )
    }
    ...
    if (mode === 'preview') {
      ...
      const scheduleTitlesForBongtour = schedule.map((d: { title?: string }) => String(d.title ?? '').trim())
    ...
    const productData = {
      ...
      benefitSummary,
      highlightPointsRaw: highlightRaw,
      highlightPoints: highlightRaw,
      promotionLabelsRaw,
