...
export const KYOWONTOUR_HOTEL_UNDECIDED_PLACEHOLDER =
  '현재 숙박시설은 미정입니다. 출발 전 안내됩니다.'

const KYOWONTOUR_REVIEW_OR_PROSE_LIKE =
...
export function kyowontourHotelLineLooksUndecidedOnly(line: string): boolean {
  const t = line.replace(/\r/g, '').trim()
  if (!t) return true
  if (KYOWONTOUR_REVIEW_OR_PROSE_LIKE.test(t) && t.length > 40) return false
...
export function kyowontourHotelTextBlockIsUndecidedOnly(text: string | null | undefined): boolean {
  ...
  return lines.every((l) => kyowontourHotelLineLooksUndecidedOnly(l))
}

/** 일정표의 매일 hotelText가 모두 “미정” 수준이면 본문에서 호텔 블록을 억지로 파싱하지 않는다. */
export function kyowontourScheduleHotelsAreAllUndecided(
  ...
  for (const s of schedule) {
    ...
    if (!ht) return false
    if (!kyowontourHotelTextBlockIsUndecidedOnly(ht)) return false
  }
  return true
}

/** 리뷰·장문 본문이 호텔 플랜으로 잘못 들어온 경우 걸러낸다. */
export function kyowontourDayHotelPlansLookLikeRealHotels(plans: DayHotelPlan[]): boolean {
  ...
  for (const p of plans) {
    for (const h of p.hotels ?? []) {
      ...
      if (KYOWONTOUR_REVIEW_OR_PROSE_LIKE.test(t)) return false
      if (kyowontourHotelLineLooksUndecidedOnly(t)) continue
      if (t.length >= 2 && t.length <= 120) sawReal = true
    }
    const raw = (p.raw ?? '').trim()
    if (raw.length > 200 && KYOWONTOUR_REVIEW_OR_PROSE_LIKE.test(raw)) return false
  }
  return sawReal
}

export function kyowontourPlaceholderDayHotelPlans(): DayHotelPlan[] {
  return [{ dayIndex: 1, label: '숙박 안내', hotels: [KYOWONTOUR_HOTEL_UNDECIDED_PLACEHOLDER] }]
}
...
export function dayHotelPlansFromSchedule(
  ...
  for (const s of schedule) {
    const ht = s.hotelText?.trim()
    if (!ht) continue
    if (kyowontourHotelTextBlockIsUndecidedOnly(ht)) continue
...
export function resolveDayHotelPlansForPublic(
  ...
  const schedAllUndecided = kyowontourScheduleHotelsAreAllUndecided(schedule)

  const fromStruct = normalizeDayHotelPlansFromUnknown(structuredPlans ?? [])
  if (fromStruct.length) {
    if (kyowontourDayHotelPlansLookLikeRealHotels(fromStruct)) return fromStruct
  ...
  if (schedAllUndecided) return kyowontourPlaceholderDayHotelPlans()

  const blob = [hotelInfoRaw, hotelSummaryRaw].filter((x) => x?.trim()).join('\n\n')
  const parsed = parseDayHotelPlansFromSupplierText(blob)
  if (parsed.length && kyowontourDayHotelPlansLookLikeRealHotels(parsed)) return parsed
  return []
}
...
export function mergeDayHotelPlansForRegister(
  ...
  if (fromLlm.length) {
    if (kyowontourDayHotelPlansLookLikeRealHotels(fromLlm)) return fromLlm
