...
import {
  AIRTEL_BLOG_PROMPT_V1,
  AIRTEL_BLOG_PROMPT_VERSION,
  PACKAGE_BLOG_PROMPT_V1,
  PACKAGE_BLOG_PROMPT_VERSION,
  buildAirtelBlogUserJson,
  buildPackageBlogUserJson,
  type AirtelBlogLlmPayload,
  type AirtelBlogLlmV1,
...
export type GenerateNaverBlogDraftForAirtelOptions = {
  persist?: boolean
  skipIfDraftExists?: boolean
  geminiModel?: string
  /** airtel/private 가 아닌 상품도 초안을 만들 때 true */
  forceAirtelTrack?: boolean
}

export type GenerateNaverBlogDraftForAirtelOk = {
  ok: true
  blogPostId?: string
  title: string
  excerpt: string | null
...
export type GenerateNaverBlogDraftForAirtelErr = {
  ok: false
  code:
    | 'INVALID_MONTH'
    | 'NOT_FOUND'
...
export type GenerateNaverBlogDraftForAirtelResult =
  | GenerateNaverBlogDraftForAirtelOk
  | GenerateNaverBlogDraftForAirtelErr

export type GenerateMonthBlogDraftsForAirtelsOptions = {
  persist?: boolean
  skipIfDraftExists?: boolean
  geminiModel?: string
  listOptions?: ListProductsForMarketingMonthOptions
...
function airtelBlogFromParsedJson(value: unknown): AirtelBlogLlmV1 {
  if (!value || typeof value !== 'object') throw new Error('JSON root must be object')
  const o = value as Record<string, unknown>
  const title = typeof o.title === 'string' ? o.title.trim() : ''
  const body = typeof o.body === 'string' ? o.body.trim() : ''
  ...
  const excerptRaw = typeof o.excerpt === 'string' ? o.excerpt.trim() : ''
  const excerpt = excerptRaw || body.replace(/\s+/g, ' ').slice(0, 200)
  const rsRaw = Array.isArray(o.recommendedSpots) ? o.recommendedSpots : []
  const rfRaw = Array.isArray(o.recommendedFoods) ? o.recommendedFoods : []
  const recommendedSpots = rsRaw
    .filter((x): x is string => typeof x === 'string')
    .map((x) => x.trim())
    .filter(Boolean)
    .slice(0, 6)
  const recommendedFoods = rfRaw
    .filter((x): x is string => typeof x === 'string')
    .map((x) => x.trim())
    .filter(Boolean)
    .slice(0, 6)
  return { title, body, excerpt, recommendedSpots, recommendedFoods }
}
...
export async function generateNaverBlogDraftForAirtel(
  prisma: PrismaClient,
  productId: string,
  monthKey: string,
  options?: GenerateNaverBlogDraftForAirtelOptions,
): Promise<GenerateNaverBlogDraftForAirtelResult> {
  const mk = monthKey.trim()
  if (!isValidYearMonth(mk)) {
    return { ok: false, code: 'INVALID_MONTH', error: `monthKey 형식 오류: ${monthKey}` }
  }
  ...
  const skipIfDraftExists =
    options?.skipIfDraftExists !== undefined ? options.skipIfDraftExists : persist
  const forceAirtelTrack = options?.forceAirtelTrack === true

  const ctx = await getAirtelBlogContext(prisma, productId, mk)
  if (!ctx) {
    return { ok: false, code: 'NOT_FOUND', error: `Product not found: ${productId}` }
  }
  ...
  if (!forceAirtelTrack && !isAirtelPrivateProductType(pt)) {
    return {
      ...
      error:
        '자유여행(에어텔·프라이빗) 전용 생성기입니다. travel/semi 등은 forceAirtelTrack=true 로만 실행할 수 있습니다.',
    }
  }
  ...
  const payload: AirtelBlogLlmPayload = {
    geo,
    campaignMonthKey: mk,
    product: {
      title: ctx.row.title,
  ...
  const modelId = (options?.geminiModel?.trim() || getModelName()).trim()
  const model = getGenAI().getGenerativeModel({ model: modelId })
  const userText = `${AIRTEL_BLOG_PROMPT_V1}\n\n## 입력(JSON)\n${buildAirtelBlogUserJson(payload)}`

  let parsed!: AirtelBlogLlmV1
  let geminiJsonParseStrategy!: NonNullable<GenerateNaverBlogDraftForAirtelOk['geminiJsonParseStrategy']>
  let lastRawText = ''
  ...
  for (let attempt = 1; attempt <= 2; attempt++) {
    ...
    if (!pj.ok) {
      console.warn('[airtel-blog]', productId, attempt, pj.strategy, pj.errorReason)
      if (attempt === 2) {
        return {
          ok: false,
          code: 'PARSE_FAIL',
    ...
    try {
      parsed = airtelBlogFromParsedJson(pj.value)
      geminiJsonParseStrategy = pj.strategy
      break
    } catch (e) {
      const msg = e instanceof Error ? e.message : String(e)
      console.warn('[airtel-blog]', productId, attempt, pj.strategy, msg)
      if (attempt === 2) {
        return {
          ok: false,
          code: 'PARSE_FAIL',
  ...
  const created = await prisma.bongBlogPost.create({
    data: {
      ...
      countrySlug: ctx.row.country?.trim() || null,
      generationModel: modelId,
      generationPromptVersion: AIRTEL_BLOG_PROMPT_VERSION,
    },
  })
...
export async function generateMonthBlogDraftsForAirtels(
  prisma: PrismaClient,
  monthKey: string,
  options?: GenerateMonthBlogDraftsForAirtelsOptions,
): Promise<GenerateMonthBlogDraftsReport> {
  const mk = monthKey.trim()
  if (!isValidYearMonth(mk)) {
    return {
  ...
  for (const c of pool) {
    const r = await generateNaverBlogDraftForAirtel(prisma, c.productId, mk, {
      persist,
      skipIfDraftExists: skipDup,
      geminiModel: options?.geminiModel,
    })
