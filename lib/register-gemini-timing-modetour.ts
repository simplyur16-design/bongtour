/**
 * 모두투어 Gemini generateContent 운영 타이밍 로그 (Railway grep용).
 */
...
export function modetourOriginCodeForGeminiLog(paste: string): string {
  const m = paste.match(/(?:^|\n)\s*상품코드\s*\n\s*([A-Z0-9][A-Z0-9-]{4,})\s*(?:\n|$)/im)
...
export async function geminiGenerateContentWithModetourLog(
  model: GeminiModel,
  ...
  const t0 = Date.now()
  console.log(
    `[gemini-modetour] start originCode=${opts.originCode} inputCharsApprox=${inputCharsApprox} promptType=${opts.promptType}`
  )
  try {
    ...
    console.log(
      `[gemini-modetour] end originCode=${opts.originCode} elapsedMs=${elapsedMs} outputCharsApprox=${outputCharsApprox} status=ok`
    )
    return result
  } catch (e) {
    const elapsedMs = Date.now() - t0
    ...
    console.error(
      `[gemini-modetour] ${kind} originCode=${opts.originCode} elapsedMs=${elapsedMs} status=${status} message=${message}`
    )
    throw e
  }
}
