/** 하나투어 등록 전용: 비정형 옵션·쇼핑 본문 휴리스틱. `register-input-parse-hanatour`만 import. */
...
import { parseOptionalTourTableRowsFromRawText, parseShoppingStopsFromLines } from '@/lib/structured-tour-signals-hanatour'
  ...
  if (looksLikeOptionalTable) {
    ...
    if (tableRows.length > 0) {
      const rows = tableRows.map((r) => ({
        ...
        descriptionText: '',
        noteText: '',
        alternateScheduleText: r.alternateScheduleText?.trim() || undefined,
      }))
      return {
        rows,
...
/** 일정 `시내면세점 N회 방문`, 핵심 `○○ 쇼핑몰` 등 — 후보 **장소**만 행으로 보강(횟수는 별도 추출). */
function mergeHanatourNarrativeShoppingPlaces(
  rows: ShoppingStructured['rows'],
  narrativeLines: string[]
): ShoppingStructured['rows'] {
  const seen = new Set(
    rows
      .map((r) => (r.shoppingPlace || r.shoppingItem || '').replace(/\s+/g, ' ').trim().toLowerCase())
      .filter(Boolean)
  )
  const extra: ShoppingStructured['rows'] = []
  for (const line of narrativeLines) {
    const mv = line.match(/^(.+?)\s*(\d+)\s*회\s*방문\s*$/)
    if (mv) {
      const place = mv[1]!.replace(/\s+/g, ' ').trim().replace(/^[·•\-\s]+/, '')
      ...
      extra.push({
        ...
        durationText: '',
        refundPolicyText: '',
        noteText: 'hanatour-narrative-visit-place',
      })
    }
    const mall = line.match(/([가-힣A-Za-z0-9・·\s]{2,36}?)\s*쇼핑몰/)
    if (mall) {
      ...
      extra.push({
        ...
        durationText: '',
        refundPolicyText: '',
        noteText: 'hanatour-narrative-mall-place',
      })
    }
  }
  return [...rows, ...extra]
...
function finalizeHanatourUnstructuredShopping(
  section: string,
  rows: ShoppingStructured['rows'],
  countLine: string
): ShoppingStructured {
  const narrativeLines = section.split(/\n/).map(shoppingCleanLine).filter(Boolean)
  const merged = mergeHanatourNarrativeShoppingPlaces(rows, narrativeLines)
  const v = extractHanatourShoppingVisitCountFromFullBodyText(section.replace(/\r/g, '\n'))
  const shoppingCountText = v != null ? `쇼핑 ${v}회` : countLine
  return {
    rows: merged,
    shoppingCountText,
    reviewNeeded: merged.length > 0 && merged.every((r) => !r.shoppingItem && !r.shoppingPlace),
    reviewReasons:
      merged.length === 0
        ? []
        : merged.every((r) => !r.shoppingItem && !r.shoppingPlace)
          ? ['쇼핑 row 핵심 열 복원 실패']
          : [],
  }
}
...
export function parseUnstructuredShoppingBodyForRegister(section: string): ShoppingStructured {
  const tsvCandidate =
    (/\t/.test(section) || /\s{3,}/.test(section)) &&
    (/회차/i.test(section) || /^구분\t|(?:^|\n)구분\t/m.test(section) || /(?:^|\n)구분\s{2,}/m.test(section)) &&
  ...
  if (tsvCandidate) {
    ...
    if (stops.length > 0) {
      ...
      return finalizeHanatourUnstructuredShopping(section, rows, count)
    }
  }
