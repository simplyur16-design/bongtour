...
/** 일정 전용 호출 출력 상한 (10~12일·식사·호텔 필드 포함 시 여유). `GEMINI_REGISTER_LOTTETOUR_SCHEDULE_EXTRACT_MAX_OUTPUT_TOKENS`가 있으면 우선. */
const SCHEDULE_EXTRACT_MAX_OUTPUT_BASE = Math.max(
  8192,
  Math.min(65536, Number(process.env.GEMINI_SCHEDULE_EXTRACT_MAX_OUTPUT_TOKENS) || 24576)
)
const lotteSchedOut = Number(process.env.GEMINI_REGISTER_LOTTETOUR_SCHEDULE_EXTRACT_MAX_OUTPUT_TOKENS)
export const SCHEDULE_EXTRACT_MAX_OUTPUT_TOKENS =
  Number.isFinite(lotteSchedOut) && lotteSchedOut > 0
    ? Math.max(8192, Math.min(65536, Math.floor(lotteSchedOut)))
    : SCHEDULE_EXTRACT_MAX_OUTPUT_BASE
...
function parseScheduleRowsFromLlmJson(
  ...
  if (opts.strictDay != null) {
    ...
    if (row) return applyLottetourScheduleImageKeywordsToRows([{ ...row, day: want }])
    return []
  }
  ...
  return applyLottetourScheduleImageKeywordsToRows(out)
}
