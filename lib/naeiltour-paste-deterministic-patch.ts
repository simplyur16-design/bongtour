...
/** URL·붙여넣기·view.asp HTML에서 good_cd 후보 추출. */
export function parseNaeiltourGoodCdFromBlob(blob: string): string | null {
  return (
    blob.match(/[?&]good_cd=([^&#\s'"<>]+)/i)?.[1]?.trim() ??
    blob.match(/name=["']good_cd["'][^>]*value=["']([^"']+)["']/i)?.[1]?.trim() ??
    blob.match(/value=["']([^"']+)["'][^>]*name=["']good_cd["']/i)?.[1]?.trim() ??
    null
  )
}

/** URL·붙여넣기 blob에서 goodCd·eventSeq 추출 — trip anchors·merge 공용. */
export function extractNaeiltourMasterIdsFromBlob(blob: string): {
  goodCd: string | null
  eventSeq: string | null
} {
  const goodCd = parseNaeiltourGoodCdFromBlob(blob)
  const eventSeq =
    blob.match(/[?&]event_seq=([^&\s#'"<>]+)/i)?.[1]?.trim() ??
    blob.match(/name=["']event_seq["'][^>]*value=["']([^"']+)["']/i)?.[1]?.trim() ??
    blob.match(/value=["']([^"']+)["'][^>]*name=["']event_seq["']/i)?.[1]?.trim() ??
    null
...
export function mergeNaeiltourDeterministicFieldsFromPaste(parsed: RegisterParsed, rawText: string): RegisterParsed {
  ...
  const px = extractNaeiltourThreeSlotPricesFromBlob(blob)
  ...
  const ids = extractNaeiltourMasterIdsFromBlob(blob)
  if (ids.goodCd && !(next.goodCd ?? '').trim()) {
    next = { ...next, goodCd: ids.goodCd }
  }
