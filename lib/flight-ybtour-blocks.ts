/**
 * 노랑풍선 정형 항공: `출발`/`도착` 블록 + 항공사 중복·편명·도시·일시.
 *
 * 붙여넣기 예:
...
export type YbtourFlightBlockParse = {
...
function isYbtourArriveAnchorLine(line: string): boolean {
  const t = line.replace(/\s+/g, ' ').trim()
...
function isYbtourInlineDepartLine(line: string): boolean {
  const t = line.replace(/\s+/g, ' ').trim()
  if (!/^출발\s+/i.test(t)) return false
  if (isYbtourDepartAnchorLine(line)) return false
  return t.length > 12
}

function isYbtourInlineArriveLine(line: string): boolean {
  const t = line.replace(/\s+/g, ' ').trim()
  if (!/^도착\s+/i.test(t)) return false
  if (isYbtourArriveAnchorLine(line)) return false
  return t.length > 12
}

function tryParseYbtourFlightBlocksInline(lines: string[]): YbtourFlightBlockParse | null {
  const departIdxs: number[] = []
  const arriveIdxs: number[] = []
  for (let i = 0; i < lines.length; i++) {
    if (isYbtourInlineDepartLine(lines[i]!)) departIdxs.push(i)
    if (isYbtourInlineArriveLine(lines[i]!)) arriveIdxs.push(i)
  }
 ...
 */
function parseYbtourMainScheduleLegLine(line: string): Partial<FlightLeg> | null {
  const parts = line
    .split(/\s*\/\s*/)
    .map((p) => p.trim())
    .filter(Boolean)
...
function extractYbtourAirlineBeforeMainSchedule(blob: string): string | null {
...
export function parseYbtourTravelMainScheduleFlight(blob: string): YbtourFlightBlockParse | null {
  ...
  const outbound = parseYbtourMainScheduleLegLine(outM[1]!.trim())
  const inbound = parseYbtourMainScheduleLegLine(inM[1]!.trim())
  if (!outbound?.flightNo || !inbound?.flightNo) return null
  const airlineName = extractYbtourAirlineBeforeMainSchedule(flat)
  return { airlineName, outbound, inbound }
}

export function tryParseYbtourFlightBlocks(section: string): YbtourFlightBlockParse | null {
  ...
  const arriveIdxs: number[] = []
  for (let i = 0; i < lines.length; i++) {
    if (isYbtourDepartAnchorLine(lines[i]!)) departIdxs.push(i)
    if (isYbtourArriveAnchorLine(lines[i]!)) arriveIdxs.push(i)
  }
  if (!departIdxs.length || !arriveIdxs.length) {
    const inline = tryParseYbtourFlightBlocksInline(lines)
  ...
  for (const idxOut of departIdxs) {
    for (const idxIn of arriveIdxs) {
      ...
      const idxOut2 = lines.findIndex((l, i) => i > idxIn && isYbtourDepartAnchorLine(l))
  ...
  if (!cands.length) {
    return tryParseYbtourFlightBlocksInline(lines)
  }
  cands.sort((a, b) => a.score - b.score)
  const best = cands[cands.length - 1]!
  return {
 ...
 */
export function parseYbtourTravelMainScheduleFlightFromTable(blob: string): YbtourFlightBlockParse | null {
  const flat = blob.replace(/\r/g, '\n')
  ...
  return tryParseYbtourFlightBlocks(slice)
}

/** generic 선호 레그용 한 줄 요약 */
export function ybtourSynthesizePreferredRaw(leg: Partial<FlightLeg>, label: '가는편' | '오는편'): string {
  const p = [
    label,
    leg.flightNo,
