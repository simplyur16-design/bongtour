...
export async function buildProductPublicDetailRenderModel(
  travelProduct: ProductDetailViewRow,
  fitMaster: FitMasterWithDays | null,
): Promise<ProductPublicDetailRenderModel> {
  const {
  ...
  if (isAirtelItineraryView) {
    ...
    return {
      ...
      seo: buildSeoBundle(seoOffers),
      registrationStatus: travelProduct.registrationStatus,
    }
  }
  ...
  return {
    ...
    seo: buildSeoBundle(seoOffers),
    registrationStatus: travelProduct.registrationStatus,
    departuresForSeo: departures.map((d) => ({
