...
} from '@/lib/register-schedule-extract-lottetour'
import {
 ...
 * `GEMINI_REGISTER_LOTTETOUR_FULL_MAX_OUTPUT_TOKENS` > `GEMINI_REGISTER_FULL_MAX_OUTPUT_TOKENS` > 기본 98304.
 * 파싱 실패·MAX_TOKENS 시 동일 세션에서 1회 1.5배(상한 131072) 자동 재호출.
 */
const LOTTETOUR_REGISTER_FULL_MAX_OUTPUT_CAP = 131072
...
function resolveLottetourRegisterFullMaxOutputTokens(): number {
  ...
  if (Number.isFinite(full) && full > 0) {
    return Math.max(
      LOTTETOUR_REGISTER_FULL_MAX_OUTPUT_FLOOR,
      Math.min(LOTTETOUR_REGISTER_FULL_MAX_OUTPUT_CAP, Math.trunc(full))
    )
  }
  return 98304
}

const REGISTER_FULL_MAX_OUTPUT_TOKENS = resolveLottetourRegisterFullMaxOutputTokens()

function bumpLottetourMainMaxOutputTokens(current: number): number {
  return Math.min(LOTTETOUR_REGISTER_FULL_MAX_OUTPUT_CAP, Math.ceil(current * 1.5))
}

function lottetourMainParseFailureSuggestsTruncationOrBrokenJson(
...
function lottetourShouldAttemptJsonRepairAfterMain(
  finishReason: string | undefined,
  text: string,
...
/** preset 없을 때 비표시 — 롯데관광은 전용 resolveDirectedFlightLines 주입 전제(R-4-C) */
...
const REGISTER_BRAND = 'lottetour' as const
...
/** 전용 입력란 비어 있을 때 본문·regex·LLM 해당 축 미사용 — 롯데관광(lottetour) 이 파일 전용 */
function lottetourClearLlmWhenDedicatedPasteEmpty(
...
function lottetourBlankSignalsWhenDedicatedPasteEmpty(
...
/** 일정 선추출 LLM에만 전달 — 롯데관광 SSR·식사·호텔 [미정] 힌트 */
const LOTTETOUR_SCHEDULE_EXTRACT_ADDENDUM = [
  '롯데관광: 일차 헤더가 "N일차 YYYY년 MM월 DD일 요일" 또는 "DAY N"(SSR) 형태일 수 있다.',
  '식사 [조식]/[중식]/[석식] 태그는 breakfastText/lunchText/dinnerText에 나눠 넣는다.',
  '호텔 행에 [미정]이 있으면 hotelText에 원문을 보존한다.',
].join('\n')

const REGISTER_PROMPT_LOTTETOUR_SUPPLEMENT = `# [롯데관광(lottetour) 본문 — 추가 힌트]
- 식별자: godId(숫자 마스터), evtCd(행사코드 예 E01A260624KE007), 화면 상품코드(E01A-0765222 등), categoryMenuNo URL 4뎁스 { no1, no2, no3, no4 }.
- 일정: DAY 1~N SSR 또는 한글 일차+양력 날짜 헤더. schedule 길이·day와 본문 일수를 맞출 것.
- 항공: 한국출발/한국도착 한 줄·시각. KE0000 등 플레이스홀더 편명은 본문 그대로 outboundFlightNo/inboundFlightNo에 두고 임의 확정 금지. 검수는 fieldIssues info.
- 가격: 성인·소아(연령 구간 문구)·유아·유류할증료(포함/불포함 문맥)를 productPriceTable·prices·included/excluded에 반영. 선택관광 €는 currency "EUR".
- 좌석 승급(비즈니스 +500만원 등): seatUpgradeOptions[] 원문 줄.
- 호텔: 도시별 후보 [미정], "총 N개의 예정 숙박시설" → hotelList[]·hotelNames·hotelSummary.
...
${REGISTER_PROMPT_LOTTETOUR_SUPPLEMENT}
...
- 꼭 확인하세요: mustKnowItems[]/mustKnowRaw 는 **공급사 원문([PASTED REQUIRED CHECKS]·본문 해당 구간) 우선** 구조화. 상담 키워드/불포함/보험/유류/현지경비 반복 금지.
  ...
  - 원문에 해당 정보가 없으면 mustKnowItems는 비우거나 최소화. **검색·외부 사실 보완은 이 Gemini 응답에서 하지 말 것**(서버 후처리).
...

{
  "originSource": "string",
  "originCode": "string",
  "godId": "string or null",
  "evtCd": "string or null",
  "categoryMenuNo": { "no1": "string or null", "no2": "string or null", "no3": "string or null", "no4": "string or null" },
  "hotelList": [{ "city": "string", "name": "string", "status": "확정|미정" }],
  "seatUpgradeOptions": ["string"],
  "meetingInfo": { "location": "string", "time": "string or null" },
  "tourLeader": { "available": true, "feeRaw": "string or null", "feeAmount": null, "feeCurrency": "EUR or null" },
  "title": "string",
  "destination": "string",
  "duration": "string",
...
export async function parseForRegisterLlmLottetour(
  rawText: string,
  ...
  if (!options?.presetDetailBody) {
    throw new Error(
      'parseForRegisterLlmLottetour: presetDetailBody가 필요합니다. register-parse-lottetour 및 전용 parse-and-register-lottetour 경로만 사용하세요.'
    )
  }
  ...
  console.info(
    `[lottetour][timing] section-repairs-total +${Date.now() - tSectionRepairs}ms maxRepairs=${maxSectionRepairs} used=${sectionRepairsUsed} forPreview=${Boolean(options?.forPreview)}`
  )
  ...
  if (expectedDaysForSchedule != null && expectedDaysForSchedule >= 1) {
    ...
    const sr = await runScheduleExtractLlm(model, blockB, expectedDaysForSchedule, {
      logLabel: forPreview
        ? 'parseForRegisterLlmLottetour-schedule-first-preview'
        : 'parseForRegisterLlmLottetour-schedule-first',
      scheduleExtractAddendum: LOTTETOUR_SCHEDULE_EXTRACT_ADDENDUM,
    })
    console.info(
      `[lottetour][timing] schedule-extract-llm +${Date.now() - tScheduleExtract}ms forPreview=${forPreview} expectedDays=${expectedDaysForSchedule} gotRows=${sr.rows.length}`
    )
  ...
  const logMainAttempt = (isRetry: boolean) => {
    const te = text.trimEnd()
    console.info('[lottetour-llm] main Gemini register response', {
      forPreview,
      isRetry,
  ...
  console.info(
    `[lottetour][timing] main-generateContent +${Date.now() - tMainGen}ms forPreview=${forPreview} maxOutTokens=${mainLlmMaxOutputTokens}`
  )
  if (forPreview) {
    const te = text.trimEnd()
    console.info('[lottetour-llm] preview Gemini response shape', {
      finishReason: finishReason ?? null,
      endsWithClosingBrace: te.endsWith('}'),
      charLength: text.length,
      minimalPrompt: true,
  ...
  if (finishReason === 'MAX_TOKENS') {
    console.error('[lottetour-llm] Gemini finishReason=MAX_TOKENS (출력 상한 도달·JSON 잘림 가능)', {
      forPreview,
  ...
  let didLottetourMainOutputBumpRetry = false
  options?.onTiming?.('parse-start')
  try {
    if (forPreview) {
      const previewLlm = parseLlmJsonObject<Record<string, unknown>>(text, {
        logLabel: 'parseForRegisterLlmLottetour-preview-minimal',
      })
      ...
      raw = parseLlmJsonObject<RegisterGeminiLlmJson>(text, { logLabel: 'parseForRegisterLlmLottetour' })
    }
    ...
    const canBumpMain =
      !forPreview &&
      lottetourMainParseFailureSuggestsTruncationOrBrokenJson(finishReason, text, firstParseError)
    if (canBumpMain) {
      const bumped = bumpLottetourMainMaxOutputTokens(mainLlmMaxOutputTokens)
      if (bumped > mainLlmMaxOutputTokens) {
        didLottetourMainOutputBumpRetry = true
        if (options?.llmCallMetrics) options.llmCallMetrics.mainLlm += 1
        ...
        logMainAttempt(true)
        console.info(
          `[lottetour][timing] main-generateContent-retry +${Date.now() - tMainRetry}ms forPreview=${forPreview} maxOutTokens=${mainLlmMaxOutputTokens}`
        )
        try {
          raw = parseLlmJsonObject<RegisterGeminiLlmJson>(text, { logLabel: 'parseForRegisterLlmLottetour-retry-bumped' })
          options?.onTiming?.('after-parse')
        } catch (retryErr) {
  ...
  if (
    ...
    !forPreview &&
    finishReason === 'MAX_TOKENS' &&
    !didLottetourMainOutputBumpRetry
  ) {
    const bumped = bumpLottetourMainMaxOutputTokens(mainLlmMaxOutputTokens)
    if (bumped > mainLlmMaxOutputTokens) {
      didLottetourMainOutputBumpRetry = true
      ...
      logMainAttempt(true)
      console.info(
        `[lottetour][timing] main-generateContent-retry-maxtokens +${Date.now() - tMainRetryMax}ms forPreview=${forPreview} maxOutTokens=${mainLlmMaxOutputTokens}`
      )
      try {
        raw = parseLlmJsonObject<RegisterGeminiLlmJson>(text, {
          logLabel: 'parseForRegisterLlmLottetour-retry-maxtokens',
        })
        options?.onTiming?.('after-parse')
  ...
  if (raw === undefined) {
    ...
    const shouldAttemptJsonRepair = lottetourShouldAttemptJsonRepairAfterMain(finishReason, text, firstParseError)
    if (!shouldAttemptJsonRepair) {
      const te = text.trimEnd()
      console.error('[lottetour-llm] LLM JSON parse failed; repair skipped (truncation/형식 정책)', {
        finishReason,
        length: text.length,
        endsWithClosingBrace: te.endsWith('}'),
    ...
    console.error('[lottetour-llm] LLM JSON parse failed; attempting one repair generateContent call', {
      finishReason,
      length: text.length,
      firstError: firstParseError,
    })
...
${text.slice(0, 16000)}`
    ...
    console.info(
      `[lottetour][timing] json-repair-generateContent +${Date.now() - tJsonRepairGen}ms forPreview=${forPreview}`
    )
    ...
    try {
      if (forPreview) {
        const previewLlm = parseLlmJsonObject<Record<string, unknown>>(text, {
          logLabel: 'parseForRegisterLlmLottetour-preview-minimal-repair',
        })
        const det = buildPreviewDeterministicRegisterRaw({
          detailBody,
          blockB,
        ...
        raw = parseLlmJsonObject<RegisterGeminiLlmJson>(text, { logLabel: 'parseForRegisterLlmLottetour-repair-retry' })
      }
  ...
  lottetourClearLlmWhenDedicatedPasteEmpty(raw, pb)
  ...
  const titleTrimmed =
    supplierListingTitleRaw && supplierListingTitleRaw.length >= 10
      ? normalizeLottetourRegisterTitleMinimalLocal(supplierListingTitleRaw)
      : normalizeLottetourRegisterTitleMinimalLocal(llmTitleRaw) || llmTitleRaw || '상품명 없음'
