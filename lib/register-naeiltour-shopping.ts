/**
 * 내일투어: 옵션 블록·메타가 쇼핑 행으로 승격되지 않게 필터 + 쇼핑 횟수 정합.
...
export function parseNaeiltourShoppingPasteTab(raw: string): ShoppingStructured | null {
...
function parseNaeiltourShoppingVisitCount(hay: string): number | null {
...
export function naeiltourShoppingRowLooksPlausible(r: ShoppingStructured['rows'][number]): boolean {
...
export function sanitizeNaeiltourShoppingStructured(
  ...
  if (paste) {
    const tabbed = parseNaeiltourShoppingPasteTab(paste)
    if (tabbed && tabbed.rows.length > 0) {
      const filtered = tabbed.rows.filter(naeiltourShoppingRowLooksPlausible)
  ...
  const vc = parseNaeiltourShoppingVisitCount(shoppingSection)
  let shoppingCountText = structured.shoppingCountText ?? ''
  if (!shoppingCountText.trim() && vc != null && vc > 0) {
    shoppingCountText = `쇼핑 ${vc}회`
  }
  const filtered = structured.rows.filter(naeiltourShoppingRowLooksPlausible)
  return {
    ...structured,
    rows: filtered,
...
export function finalizeNaeiltourRegisterParsedShopping(parsed: RegisterParsed): RegisterParsed {
  const st = parsed.detailBodyStructured?.shoppingStructured
  if (!st) return parsed
  const plausible = st.rows.filter(naeiltourShoppingRowLooksPlausible)
  const n = parseNaeiltourShoppingVisitCount(st.shoppingCountText ?? '') ??
    parseNaeiltourShoppingVisitCount(parsed.shoppingSummaryText ?? '')
  const visitFromRows = (() => {
