...
- schedule[].routeText: 그날 방문 도시·장소를 본문 순서 그대로 ' - ' (공백-하이픈-공백)로 연결한 한 줄 경로. 한국어로 작성. 본문에 한국어 지명이 있으면 그대로 사용. 영문 지명만 있으면 한국어 음역 또는 한국에서 통용되는 한국어 표기. 예: 인천 - 부다페스트 - 나지카니자 / 인천 - 아디스아바바 - 빅토리아 폭포 / JFK공항 - 뉴욕 - 덤보 - 브루클린브릿지(조망) / 스플리트 - 두브로브니크. [조망], [차창관광], [외부관람], [선택관광] 태그는 (조망), (차창), (외부관람), (선택관광)로 보존. 공항/호텔/식당도 방문 장소면 포함. 본문에 없는 장소를 만들지 말 것. 빈 일정이면 null.`
