...
export type PortoneOutcome = "authorized" | "captured" | "failed" | "cancelled";

export type ProcessPortonePaymentResult =
...
export type ProcessPortonePaymentInput = {
  providerEventId: string;
  paymentAttemptId: string;
  outcome: PortoneOutcome;
  amountKrw?: number;
...
async function insertProviderEventIfNew(
  ...
  const r = await client.query<{ id: string }>(
    `INSERT INTO bongsim_payment_provider_event (provider, provider_event_id, payment_attempt_id, order_id, payload_json)
     ...
     RETURNING id`,
    [SIMPLYUR_PORTONE_PROVIDER_ID, provider_event_id, payment_attempt_id, order_id, JSON.stringify(payload)],
  );
...
export async function processPortonePaymentOutcome(
  input: ProcessPortonePaymentInput,
): Promise<ProcessPortonePaymentResult> {
  ...
  try {
    ...
    const a = await client.query<AttemptRow>(
      `SELECT payment_attempt_id, order_id, status, amount_krw, provider, provider_session_id
    ...
    if (input.outcome === "failed") {
      if (cur !== "captured") await updateAttemptStatus("failed");
      await client.query("COMMIT");
      return { ok: true, duplicate: false };
    }

    if (input.outcome === "cancelled") {
      if (cur !== "captured") await updateAttemptStatus("cancelled");
      await client.query("COMMIT");
      return { ok: true, duplicate: false };
    }
    ...
    if (attemptAmount !== grand || toInt(input.amountKrw) !== grand) {
      await setLastError("amount_mismatch", "portone_amount_neq_order_total");
      await client.query("COMMIT");
      return { ok: false, reason: "amount_mismatch" };
    }
    ...
    const ref = input.paymentReference ?? attempt.provider_session_id ?? `portone_${input.providerEventId}`;
    await client.query(
      `UPDATE bongsim_order
       SET status = 'paid',
           ...
           updated_at = now()
       WHERE order_id = $1`,
      [order.order_id, ref, grand, SIMPLYUR_PORTONE_PROVIDER_ID],
    );
    ...
    // REGRESSION-FREEZE[bongsim-sms-drain-on-web]: web drain after simplyur pay — manifest
    ensureOrderPaidOutboxDrainAfterEnqueue();

    return { ok: true, duplicate: false };
  } catch (e) {
    console.error("[simplyur:portone:process]", e);
    try {
      await client.query("ROLLBACK");
    } catch {
      /* ignore */
