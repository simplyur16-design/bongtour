...
import {
  normalizeNaeiltourPasteForScheduleExtract,
  sliceNaeiltourItineraryBodyForDayMarkers,
} from '@/lib/naeiltour-paste-normalize-for-register'
...
import {
  deriveNaeiltourScheduleDayHeaderTitle,
  shouldReplaceNaeiltourScheduleDayTitle,
} from '@/lib/naeiltour-schedule-day-header-title'
...
const NAEILTOUR_DAY_HEADER_LINE_RE = /^\s*(\d{1,2})\s*일차(?:\s|$)/i
/** 본문 전체 스캔: 줄 시작 또는 개행 직후 `N일차` */
const NAEILTOUR_DAY_HEADER_GLOBAL_RE = /(?:^|\n)\s*(\d{1,2})\s*일차(?=\s*(?:\n|\r|$))/gi
...
function scanNaeiltourAllDayHeaders(s: string): Array<{ day: number; headerStart: number; bodyStart: number }> {
  const matches: Array<{ day: number; headerStart: number; bodyStart: number }> = []
  let m: RegExpExecArray | null
  const re = new RegExp(NAEILTOUR_DAY_HEADER_GLOBAL_RE.source, 'gi')
...
function getNaeiltourDetailSectionLabelEnd(s: string): number {
...
function naeiltourFirstDayBlockLooksDetailed(s: string, bodyStart: number, nextHeaderStart: number): boolean {
 ...
 */
function findFirstNaeiltourDay1MatchIndex(
  ...
  for (const { m, i } of candidates) {
    ...
    if (naeiltourFirstDayBlockLooksDetailed(s, m.bodyStart, end)) return i
  }
...
export function findNaeiltourAscendingDayHeaderChain(
  haystack: string
): Array<{ day: number; headerStart: number; bodyStart: number }> {
  const s = haystack.replace(/\r\n/g, '\n')
  const matches = scanNaeiltourAllDayHeaders(s)
  const labelEnd = getNaeiltourDetailSectionLabelEnd(s)
  const minHeaderStart = labelEnd > 0 ? labelEnd : 0
  const first1 = findFirstNaeiltourDay1MatchIndex(matches, s, minHeaderStart)
  if (first1 < 0) return []
...
function extractHotelFromNaeiltourBlock(block: string): string | null {
...
function extractMealsFromNaeiltourBlock(block: string): Partial<RegisterScheduleDay> {
...
function extractNaeiltourDayDateIso(block: string): string | null {
...
export function naeiltourScheduleDayFromPastedBlock(day: number, block: string): RegisterScheduleDay {
  const dateText = extractNaeiltourDayDateIso(block)
  const lines = block.split(/\r?\n/).map((l) => l.trim()).filter(Boolean)
  const useful: string[] = []
  for (const l of lines) {
    if (NAEILTOUR_DAY_HEADER_LINE_RE.test(l)) continue
  ...
  if (shouldReplaceNaeiltourScheduleDayTitle(title, descFinal)) {
    const derived = deriveNaeiltourScheduleDayHeaderTitle({
      day,
      title: '',
  ...
  const meals = extractMealsFromNaeiltourBlock(block)
  const hotelText = extractHotelFromNaeiltourBlock(block)
...
export function buildNaeiltourScheduleFromPastedText(pastedBody: string): RegisterScheduleDay[] {
  const sliced = sliceNaeiltourItineraryBodyForDayMarkers(pastedBody)
  const normalized = normalizeNaeiltourPasteForScheduleExtract(sliced)
  const chain = findNaeiltourAscendingDayHeaderChain(normalized)
  if (!chain.length) return []
  const full = pastedBody.replace(/\r\n/g, '\n')
  const allHdr = scanNaeiltourAllDayHeaders(full)
  ...
  for (let i = 0; i < chain.length; i++) {
    ...
    out.push(naeiltourScheduleDayFromPastedBlock(day, block))
  }
  return out
}

/** LLM `parsed.schedule`에 없는 day는 본문 보조 행으로 추가. 기존 day는 식사·호텔만 보강 */
export function mergeMissingNaeiltourScheduleDays(
  parsed: RegisterParsed,
  pastedBody: string
): RegisterParsed {
  const bodyRows = buildNaeiltourScheduleFromPastedText(pastedBody)
...
export function sanitizeNaeiltourScheduleRowExpression(row: RegisterScheduleDay): RegisterScheduleDay {
...
export function augmentNaeiltourScheduleExpressionParsed(
  parsed: RegisterParsed,
  ...
  const cleaned = sched.map((r) => sanitizeNaeiltourScheduleRowExpression(stripCounselingTermsFromScheduleRow(r)))
  const titled = cleaned.map((r) => {
    const title = String(r.title ?? '').trim()
    const description = String(r.description ?? '').trim()
    if (!shouldReplaceNaeiltourScheduleDayTitle(title, description)) return r
    const nextTitle = deriveNaeiltourScheduleDayHeaderTitle({
      day: r.day,
      title,
      description,
 ...
 */
export function finalizeNaeiltourItineraryDayDraftsFromSchedule(
