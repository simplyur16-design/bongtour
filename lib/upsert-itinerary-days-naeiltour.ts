/**
 * ItineraryDay: 내일투어(naeiltour) 등록/저장 경로용 — 공급사 원문 일정표 정본(raw/source-of-truth) 계층.
...
export function registerScheduleToDayInputs(
  ...
  return schedule.flatMap((s): ItineraryDayInput[] => {
    ...
    const headerTitle =
      headerTitleRaw && !shouldReplaceNaeiltourScheduleDayTitle(headerTitleRaw, description)
        ? headerTitleRaw
        : deriveNaeiltourScheduleDayHeaderTitle({
            day,
