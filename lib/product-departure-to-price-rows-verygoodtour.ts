/**
 * 참좋은여행(verygood) 경로용 — 공개 상품 상세는 레거시 ProductPrice[] 를 직렬화해 쓰는데,
...
export function mergeProductPriceRowsWithBodyPriceTable(
  ...
  /** 본문 유아 단가만 있고 mergeInfant가 꺼진 경우에도, 빈 슬롯 채움 패스는 돌려야 한다 */
  const canFillEmptyInfantFromBody =
    table.infantPrice != null &&
    table.infantPrice > 0 &&
    rows.some((r) => positiveSlot(r.priceInfant ?? r.infant) == null)
  ...
  /**
   * 참좋은 본문 `productPriceTable.infantPrice`는 상품 단위 SSOT인데,
   * `mergeInfant`가 infantVaries·adultVaries 조합으로 꺼지면 행에 유아가 안 실려 공개 UI 유아 칸이 비는 경우가 있다.
   * 이미 행에 양의 유아 단가가 있으면 유지하고, 비어 있을 때만 본문 값을 싣는다.
   */
  const bodyInfant = table.infantPrice
  if (bodyInfant != null && bodyInfant > 0 && merged.length > 0) {
    merged = merged.map((r) => {
      if (positiveSlot(r.priceInfant ?? r.infant) != null) return r
      return { ...r, infant: bodyInfant, priceInfant: bodyInfant }
    })
  }
