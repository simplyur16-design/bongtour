/**
 * 노랑풍선(ybtour) 전용: 일차 헤더 `title`(장소·동선 요약)만 생성.
 * description / 식사·숙소 / imageKeyword는 변경하지 않는다.
 */

export type YbtourHeaderTitleInput = {
  day: number
  /** LLM·추출 기존 title (참고만) */
  title: string
  /** 동선·관광 근거 텍스트 (읽기 전용) */
...
function extractPlacesInOrder(hay: string): string[] {
  ...
  for (let i = 0; i < h.length; i++) {
    for (const name of YBTOUR_PLACE_NAMES) {
...
export function shouldReplaceYbtourScheduleDayTitle(title: string, description: string): boolean {
 ...
 */
export function deriveYbtourScheduleDayHeaderTitle(input: YbtourHeaderTitleInput): string {
