/** [naeiltour] register-flight-evidence-supplier */
/**
...
function naeiltourAirportWithAdjacentTime(
  block: string[],
  which: 'first' | 'last',
...
function naeiltourSnippet(
  ...
  return trySources(parsed, preview, (lines) => {
    ...
    const depIncheon = naeiltourAirportWithAdjacentTime(depBlock, 'first', (l) => l.includes('인천'))
    const arrIncheon = naeiltourAirportWithAdjacentTime(arrBlock, 'last', (l) => l.includes('인천'))
...
export function buildSupplierFlightSnippet(
  ...
  else if (b === 'naeiltour') raw = naeiltourSnippet(kind, parsed, preview)
