/** [verygoodtour] register-flight-evidence-supplier */
/**
...
/** 노랑풍선: 공항명 줄·시각 줄이 분리된 경우 인접 줄을 묶어 evidence로 쓴다 */
function ybtourAirportWithAdjacentTime(
  block: string[],
  which: 'first' | 'last',
...
function ybtourSnippet(
  ...
  return trySources(parsed, preview, (lines) => {
    ...
    const depIncheon = ybtourAirportWithAdjacentTime(depBlock, 'first', (l) => l.includes('인천'))
    const arrIncheon = ybtourAirportWithAdjacentTime(arrBlock, 'last', (l) => l.includes('인천'))
...
export function buildSupplierFlightSnippet(
  ...
  else if (b === 'ybtour') raw = ybtourSnippet(kind, parsed, preview)
