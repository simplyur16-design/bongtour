/**
 * 롯데관광(lottetour) 전용 — 선택관광(현지옵션) 행 게이트.
 *
 * 계약
 * - 이 모듈은 롯데관광 계열 등록/붙여넣기 본문에서 추출한 선택관광 행만 판별한다. 타 공급사 표를 공용 규칙으로 흡수하지 않는다.
 ...
 *
 * 전용 차이(최소)
 * - `BAN_NAME_REGEX`의 브랜드 마일리지 안내 줄: 롯데관광 브랜드 표기를 패턴 앞쪽에 둔다(가시성·추후 분기 여지).
 */
...
const BAN_NAME_REGEX: RegExp[] = [
  ...
  // 타사·타 브랜드 마일리지 안내 줄(붙여넣기 혼재 대비).
  /롯데관광\s*마일리지|하나투어\s*마일리지|모두투어\s*마일리지|참좋은여행\s*마일리지/i,
  /인솔자\s*경비|가이드\s*경비|기사\s*경비|인솔\s*경비/i,
  /좌석\s*승급|승급\s*옵션|프리미엄\s*좌석|업그레이드\s*좌석|좌석\s*업그레이드|비즈니스\s*클래스|비지니스\s*클래스|business\s*class/i,
]
