...
import {
  normalizeYbtourPasteForScheduleExtract,
  sliceYbtourItineraryBodyForDayMarkers,
} from '@/lib/ybtour-paste-normalize-for-register-ybtour'
...
const YBTOUR_DAY_HEADER_LINE_RE = /^\s*(\d{1,2})\s*일차(?:\s|$)/i
/** 본문 전체 스캔: 줄 시작 또는 개행 직후 `N일차` */
const YBTOUR_DAY_HEADER_GLOBAL_RE = /(?:^|\n)\s*(\d{1,2})\s*일차(?=\s*(?:\n|\r|$))/gi
...
function scanYbtourAllDayHeaders(s: string): Array<{ day: number; headerStart: number; bodyStart: number }> {
  const matches: Array<{ day: number; headerStart: number; bodyStart: number }> = []
  let m: RegExpExecArray | null
  const re = new RegExp(YBTOUR_DAY_HEADER_GLOBAL_RE.source, 'gi')
...
function getYbtourDetailSectionLabelEnd(s: string): number {
...
function ybtourFirstDayBlockLooksDetailed(s: string, bodyStart: number, nextHeaderStart: number): boolean {
...
function findFirstYbtourDay1MatchIndex(
  ...
  for (const { m, i } of candidates) {
    ...
    if (ybtourFirstDayBlockLooksDetailed(s, m.bodyStart, end)) return i
  }
...
export function findYbtourAscendingDayHeaderChain(
  ...
  const matches = scanYbtourAllDayHeaders(s)
  const labelEnd = getYbtourDetailSectionLabelEnd(s)
...
export function ybtourScheduleDayFromPastedBlock(day: number, block: string): RegisterScheduleDay {
  ...
  for (const l of lines) {
    if (YBTOUR_DAY_HEADER_LINE_RE.test(l)) continue
...
export function buildYbtourScheduleFromPastedText(pastedBody: string): RegisterScheduleDay[] {
  const sliced = sliceYbtourItineraryBodyForDayMarkers(pastedBody)
  const normalized = normalizeYbtourPasteForScheduleExtract(sliced)
  const chain = findYbtourAscendingDayHeaderChain(normalized)
  if (!chain.length) return []
  const full = pastedBody.replace(/\r\n/g, '\n')
  const allHdr = scanYbtourAllDayHeaders(full)
 ...
 */
export function finalizeYbtourItineraryDayDraftsFromSchedule(
...
export function ybtourConfirmHasScheduleExpressionLayer(
  parsed: RegisterParsed,
