...
/** LLM prices[] 문자열 — upsert-product-departures-modetour trimRaw 상한과 동일 */
...
const REGISTER_BRAND = 'modetour' as const
...
function modetourClearLlmWhenDedicatedPasteEmpty(
  raw: RegisterGeminiLlmJson,
...
function modetourBlankSignalsWhenDedicatedPasteEmpty(
...
- destination 은 **여행도시** 블록의 도시 목록·제목 권역(미동부·캐나다 등)을 우선한다. 호텔·특전·폭포뷰·VIP 한 줄을 destination 으로 쓰지 말 것.
...
export async function parseForRegisterLlmModetour(
  rawText: string,
  ...
  if (forPreview) {
    ...
    console.info('[modetour-llm] preview Gemini response shape', {
  ...
  if (finishReason === 'MAX_TOKENS') {
    console.error('[modetour-llm] Gemini finishReason=MAX_TOKENS (출력 상한 도달·JSON 잘림 가능)', {
      forPreview,
  ...
  try {
    if (forPreview) {
      const previewLlm = parseLlmJsonObject<Record<string, unknown>>(text, {
        logLabel: 'parseForRegisterLlmModetour-preview-minimal',
      })
      ...
      raw = parseLlmJsonObject<RegisterGeminiLlmJson>(text, { logLabel: 'parseForRegisterLlmModetour' })
    }
    ...
    if (!shouldAttemptJsonRepair) {
      console.error('[modetour-llm] LLM JSON parse failed; repair skipped (truncation/형식 정책)', {
        finishReason,
        length: text.length,
        endsWithClosingBrace: te.endsWith('}'),
    ...
    console.error('[modetour-llm] LLM JSON parse failed; attempting one repair generateContent call', {
      finishReason,
      length: text.length,
      firstError: firstParseError,
    })
...
${text.slice(0, 16000)}`
    ...
    try {
      if (forPreview) {
        const previewLlm = parseLlmJsonObject<Record<string, unknown>>(text, {
          logLabel: 'parseForRegisterLlmModetour-preview-minimal-repair',
        })
        const det = buildPreviewDeterministicRegisterRaw({
          detailBody,
          blockB,
        ...
        raw = parseLlmJsonObject<RegisterGeminiLlmJson>(text, { logLabel: 'parseForRegisterLlmModetour-repair-retry' })
      }
  ...
  const titleTrimmed = titleResolved.title
  const supplierListingTitleRaw = titleResolved.supplierListingTitleRaw
  const destResolved = resolveModetourRegisterDestination({
  ...
  let prices: ParsedProductPrice[] = (raw.prices ?? []).map((p) => {
    ...
    const s = (k: string) => capModetourLlmPriceField(k, ext[k])
    return {
  ...
  const optionalToursLineRegex = pb.optionalTour?.trim()
    ...
    : null
  const signals = modetourBlankSignalsWhenDedicatedPasteEmpty(extractStructuredTourSignals(signalsHaystack), pb)
  ...
  const modetourTitleIssue = modetourRegisterTitleFieldIssue(titleTrimmed)
  if (modetourTitleIssue) extractionFieldIssues.push(modetourTitleIssue)
  {
