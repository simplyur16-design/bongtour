/**
 * 노랑풍선(ybtour) 관리자 등록 — **본문 축** 스냅샷만 조립한다.
 *
 * 담당: 본문 정규화, 섹션 앵커·분리·슬라이스, 호텔 본문 구조화, 포함/불포함, `raw.flightRaw` 등 슬라이스 원료,
 * 약관/예약금 경계(클립). 일정 원료는 섹션 텍스트로만 남기고 일차 배열화는 하지 않는다.
 *
 * **비담당(SSOT = 입력 파서):** 항공·선택관광/옵션·쇼핑 구조화 — `register-input-parse-ybtour` 및
 * `register-parse-ybtour`의 정형 입력란만이 구조화한다. 본문에 동일 문구가 있어도 여기서는 채우지 않는다.
 *
...
import { normalizeDetailRawText, splitDetailSections, sliceDetailBodySections } from '@/lib/detail-body-parser-utils-ybtour'
import { parseHotelSectionYbtour } from '@/lib/hotel-parser-ybtour'
...
/** 포함/불포함 구조화 입력에서 약관·취소·예약금 장문 이후는 잘라 내어 `register-ybtour-basic` 파서 오염을 막는다(SSOT: `docs/body-parser-ybtour-ssot.md`). */
export function clipYbtourIncExcInputForParse(blob: string): string {
...
export function parseDetailBodyStructuredYbtour(input: {
  rawText: string
  ...
  const hotelStructured = parseHotelSectionYbtour(hotelSection)
  const optionalToursStructured = emptyOptionalToursStructured()
  const shoppingStructured = emptyShoppingStructured()
  const incExcForParse = clipYbtourIncExcInputForParse(incExcSection)
  let includedExcludedStructured = parseYbtourIncludedExcludedSection(incExcForParse)
  if (
    ...
    includedExcludedStructured = parseYbtourIncludedExcludedSection(
      clipYbtourIncExcInputForParse(normalizedRaw)
    )
  }

  const { review, sectionReview, qualityScores, failurePatterns } = buildDetailReviewPolicyYbtour({
    sections,
    flightStructured,
    hotelStructured,
