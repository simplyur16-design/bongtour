...
} from '@/lib/lottetour-paste-normalize-for-register'
...
const Lottetour_DAY_HEADER_LINE_RE = /^\s*(\d{1,2})\s*일차(?:\s|$)/i
/** 본문 전체 스캔: 줄 시작 또는 개행 직후 `N일차` */
/**
 * 같은 줄에 `2026년 06월 24일` 등이 붙는 SSR 본문을 허용한다.
 * (`\\b`는 한글 앞뒤에서 단어 경계로 잡히지 않아 `1일차 2026년` 매치가 깨진다.)
 */
const Lottetour_DAY_HEADER_GLOBAL_RE = /(?:^|\n)\s*(\d{1,2})\s*일차(?=\s|$)/gi
...
function scanLottetourAllDayHeaders(s: string): Array<{ day: number; headerStart: number; bodyStart: number }> {
  const matches: Array<{ day: number; headerStart: number; bodyStart: number }> = []
  let m: RegExpExecArray | null
  const re = new RegExp(Lottetour_DAY_HEADER_GLOBAL_RE.source, 'gi')
...
function getLottetourDetailSectionLabelEnd(s: string): number {
...
function lottetourFirstDayBlockLooksDetailed(s: string, bodyStart: number, nextHeaderStart: number): boolean {
 ...
 */
function findFirstLottetourDay1MatchIndex(
  ...
  for (const { m, i } of candidates) {
    ...
    if (lottetourFirstDayBlockLooksDetailed(s, m.bodyStart, end)) return i
  }
...
export function findLottetourAscendingDayHeaderChain(
  haystack: string
): Array<{ day: number; headerStart: number; bodyStart: number }> {
  const s = haystack.replace(/\r\n/g, '\n')
  const matches = scanLottetourAllDayHeaders(s)
  const labelEnd = getLottetourDetailSectionLabelEnd(s)
  const minHeaderStart = labelEnd > 0 ? labelEnd : 0
  const first1 = findFirstLottetourDay1MatchIndex(matches, s, minHeaderStart)
  if (first1 < 0) return []
...
function extractHotelFromLottetourBlock(block: string): string | null {
...
function extractMealsFromLottetourBlock(block: string): Partial<RegisterScheduleDay> {
...
export function lottetourScheduleDayFromPastedBlock(day: number, block: string): RegisterScheduleDay {
  const dateText = extractLottetourDayDateIso(block)
  const lines = block.split(/\r?\n/).map((l) => l.trim()).filter(Boolean)
  const useful: string[] = []
  for (const l of lines) {
    if (Lottetour_DAY_HEADER_LINE_RE.test(l)) continue
  ...
  if (shouldReplaceLottetourScheduleDayTitle(title, descFinal)) {
    const derived = deriveLottetourScheduleDayHeaderTitle({
      day,
      title: '',
  ...
  const meals = extractMealsFromLottetourBlock(block)
  const hotelText = extractHotelFromLottetourBlock(block)
...
export function buildLottetourScheduleFromPastedText(pastedBody: string): RegisterScheduleDay[] {
  const sliced = sliceLottetourItineraryBodyForDayMarkers(pastedBody)
  const normalized = normalizeLottetourPasteForScheduleExtract(sliced)
  const chain = findLottetourAscendingDayHeaderChain(normalized)
  if (!chain.length) return []
  const full = pastedBody.replace(/\r\n/g, '\n')
  const allHdr = scanLottetourAllDayHeaders(full)
...
export function mergeMissingLottetourScheduleDays(
  ...
  const bodyRows = buildLottetourScheduleFromPastedText(pastedBody)
...
export function sanitizeLottetourScheduleRowExpression(row: RegisterScheduleDay): RegisterScheduleDay {
...
export function augmentLottetourScheduleExpressionParsed(
  ...
  if (pastedBodyText?.trim()) {
    next = mergeMissingLottetourScheduleDays(next, pastedBodyText)
  }
 ...
 */
export function finalizeLottetourItineraryDayDraftsFromSchedule(
...
export function lottetourConfirmHasScheduleExpressionLayer(
  parsed: RegisterParsed,
