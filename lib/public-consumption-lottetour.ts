/**
 * 롯데관광(lottetour) 전용 — 공개 상세 consumption resolution (선택관광·쇼핑·호텔 canonical / legacy 우선순위).
 *
 * 계약
 * - 호출부는 레거시 공용 resolution 합본·페이지 공통 모듈 선택기 없이 이 파일의 `resolve*`만 직접 호출한다.
 * - 이 파일만을 공개 상세 consumption 단일 진입으로 둔다.
 */
import { splitHotelNamesLine, type DayHotelPlan } from '@/lib/day-hotel-plans-lottetour'
