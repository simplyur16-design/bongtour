...
export function parseModetourRegisterPastedBlocksFromBody(
  ...
  const pick = (key: ModetourRegisterAdminPastedBlockKey) => {
    const v = o[key]
    return typeof v === 'string' && v.trim()
      ? v.trim().slice(0, MODETOUR_REGISTER_PASTED_BLOCK_MAX_CHARS)
      : undefined
  }
  const out: ModetourRegisterAdminPastedBlocksPartial = {}
  for (const key of MODETOUR_REGISTER_ADMIN_PASTED_BLOCK_KEYS) {
    const v = pick(key)
    if (v) out[key] = v
  }
