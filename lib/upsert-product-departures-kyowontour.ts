...
export async function loadKyowontourProductInfantFallback(
  prisma: PrismaClient,
  productId: string
): Promise<number | null> {
...
export function enrichKyowontourDepartureInputsForConfirmSave(
...
export async function upsertProductDepartures(
  ...
  const productInfantFallback = await loadKyowontourProductInfantFallback(prisma, productId)
