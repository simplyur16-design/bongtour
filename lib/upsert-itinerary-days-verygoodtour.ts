/**
 * ItineraryDay: 참좋은여행(verygood) 등록/저장 경로용 — 공급사 원문 일정표 정본(raw/source-of-truth) 계층.
...
/**
 * 일차: 1-based(1,2,3) 또는 0-based(0,1,2) 입력 모두 1-based 저장값으로 통일.
 * LLM이 0일차를 0으로 두는 경우 등록·ItineraryDay에서 행이 사라지지 않게 한다.
 */
export function coerceScheduleDayToOneBased(raw: unknown): number | null {
  const d = normalizeDay(raw)
  if (d != null) return d
  const n = Number(raw)
  if (!Number.isInteger(n) || n < 0 || n > 366) return null
  return n + 1
}
...
export function registerScheduleToDayInputs(
  ...
  return schedule.flatMap((s): ItineraryDayInput[] => {
    const day = coerceScheduleDayToOneBased(s.day) ?? 0
    if (day < 1) return []
...
export function parsedItinerariesToDayInputs(
  ...
  return itineraries.flatMap((i): ItineraryDayInput[] => {
    const day = coerceScheduleDayToOneBased(i.day) ?? 0
    if (day < 1) return []
...
export function extractedItineraryToDayInputs(
  ...
  return itinerary.flatMap((d): ItineraryDayInput[] => {
    const day = coerceScheduleDayToOneBased(d.day) ?? 0
    if (day < 1) return []
