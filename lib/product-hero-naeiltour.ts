...
function combineNaeiltourFlightDateTime(d: string | null | undefined, t: string | null | undefined): string | null {
...
function alignNaeiltourAtTextToDateIso(atText: string | null | undefined, dateIso: string | null): string | null {
 ...
 * 히어로 출발·귀국 표시는 구조화 본문(naeiltourFlightStructured)보다 이쪽만 쓴다(날짜·귀국 혼선 방지).
 */
function tryNaeiltourHeroFromDepartureKeyFactsOnly(
  ...
  const depLabel = alignNaeiltourAtTextToDateIso(dOut, calendarDep) ?? dOut
  const retLabel = (retIso ? alignNaeiltourAtTextToDateIso(dInArr, retIso) : null) ?? dInArr

  return {
    departureIso: calendarDep,
    returnIso: retIso,
    departureSource: 'naeiltour_departure_key_facts',
    returnSource: retIso ? 'naeiltour_departure_key_facts_inbound' : 'none',
 ...
 */
function tryNaeiltourHeroFromDepartureKeyFactsPartial(
  calendarDep: string | null,
  facts: DepartureKeyFacts,
  duration: string | null | undefined
): HeroTripResolved | null {
  ...
  const depLabel =
    (dOut ? alignNaeiltourAtTextToDateIso(dOut, calendarDep) ?? dOut : null) ?? formatHeroDateKorean(calendarDep)

  let retLabel: string | null = null
  if (dInArr && retIso) {
    retLabel = alignNaeiltourAtTextToDateIso(dInArr, retIso) ?? dInArr
  } else if (dInArr) {
    retLabel = dInArr
  } else if (retIso) {
  ...
  return {
    departureIso: calendarDep,
    returnIso: retIso,
    departureSource: 'naeiltour_departure_key_facts_partial',
    returnSource: retIso
      ? dInArr
        ? 'naeiltour_departure_key_facts_inbound'
        : 'naeiltour_departure_key_facts_duration'
      : 'none',
...
function tryNaeiltourHeroFromFlightStructured(
  ...
  let retSrc = 'naeiltour_flight_structured_inbound_arrival'
  if (!retIso) {
    const list = returnFromListFacts(facts)
    retIso = list.iso
  ...
  const depLabel = combineNaeiltourFlightDateTime(depIso ?? ob.departureDate, obTime)
  const retLabel = combineNaeiltourFlightDateTime(retIso ?? ib.arrivalDate, ibArrTime)
  if (!depLabel || !retLabel) return null

  return {
    departureIso: depIso,
    returnIso: retIso,
    departureSource: depIso ? 'naeiltour_flight_structured_outbound' : 'none',
    returnSource: retSrc,
    departureDisplayOverride: depLabel,
    returnDisplayOverride: retLabel,
  }
...
/**
 * 내일투어(naeiltour) 등록 미리보기·공개 상세 히어로 — 구조화 항공에서 출발·귀국 시각 오버라이드 후 공용 `resolveHeroTripDates`로 폴백.
 *
 * `disableFlightStructuredFallback`: true면 상품 본문 `naeiltourFlightStructured`를 쓰지 않는다(출발 행·`departureKeyFactsByDate`만 SSOT).
 */
export function resolveNaeiltourHeroTripDates(opts: {
  selectedDate: string | null
  fallbackPriceRowDate: string | null | undefined
  duration: string | null | undefined
  departureFacts: DepartureKeyFacts | null
  naeiltourFlightStructured?: PublicPersistedFlightStructuredDto | null | undefined
  ...
  if (opts.departureFacts) {
    const fromFacts = tryNaeiltourHeroFromDepartureKeyFactsOnly(dep, opts.departureFacts, opts.duration)
    if (fromFacts) return fromFacts
    const fromFactsPartial = tryNaeiltourHeroFromDepartureKeyFactsPartial(dep, opts.departureFacts, opts.duration)
    if (fromFactsPartial) return fromFactsPartial
  }
  if (!opts.disableFlightStructuredFallback) {
    const fromStructured = tryNaeiltourHeroFromFlightStructured(
      opts.naeiltourFlightStructured,
      dep,
      opts.duration,
      opts.departureFacts
    )
