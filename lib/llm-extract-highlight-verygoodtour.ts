/**
 * 참좋은여행(verygoodtour) 전용 — Gemini로 상품 핵심 포인트 영역 추출·정리.
...
const VERYGOOD_SYSTEM = `당신은 참좋은여행 상세 붙여넣기 원문에서 특정 구역만 분리하는 도우미입니다.
반드시 JSON 객체만 출력합니다. 마크다운·코드펜스·주석 금지.

참좋은여행 규칙:
- 추출 대상은 「POINT 1」「POINT 2」「POINT 3」 형태로 번호 매겨진 포인트 블록 전체입니다. 존재하는 POINT 번호는 빠짐없이 highlightPointsRaw에 포함합니다.
- POINT 바깥의 일반 유의사항·광고 문구는 넣지 마세요.
- 원문에 POINT 블록이 없으면 두 필드 모두 null.

환각 방지:
- 원문에 없는 내용 추가 금지.
- highlightPointsRaw는 공급사 문구 유지, 태그만 정리 가능.
- highlightPoints는 봉투어 톤 5~7줄(포인트가 많으면 가장 중요한 순으로 병합 가능하나 사실은 원문에 있어야 함), 한 줄 핵심 1개.
`
...
export async function extractHighlightFromVerygoodtourLLM(rawText: string): Promise<{
  highlightPointsRaw: string | null
  highlightPoints: string | null
} | null> {
...
`
  ...
  const model = genAI.getGenerativeModel({
    model: getModelName(),
    systemInstruction: VERYGOOD_SYSTEM,
  })
  ...
  try {
    ...
    console.warn('[llm-extract-highlight-verygoodtour]', e instanceof Error ? e.message : e)
    return null
  }
}
