...
/** 롯데관광(lottetour) 전용: 성인만 갱신 입력으로 덮고, 아동·유아는 이번 요청에 숫자가 있을 때만 갱신·없으면 DB 유지 */
function pickPreservedChildInfantPriceLottetour(
...
function pickPreservedInfantPriceLottetour(
  incoming: number | null | undefined,
  existing: number | null | undefined
): number | null {
 ...
 */
function resolveLottetourChildSlotPrice(
  incoming: number | null | undefined,
  ...
  return pickPreservedChildInfantPriceLottetour(incoming, previous)
}

/** 동일 상품의 다른 출발행·ProductPrice에서 유아 단가 후보를 찾는다(신규 출발일 행 보강). */
export async function loadLottetourProductInfantFallback(
  prisma: PrismaClient,
  productId: string
): Promise<number | null> {
...
export function enrichLottetourDepartureInputsForConfirmSave(
...
export async function upsertProductDepartures(
  ...
  for (const { dep: d, departureDate } of pairs) {
    ...
    const childBedPrice = resolveLottetourChildSlotPrice(d.childBedPrice, adultPrice, previous?.childBedPrice)
    const childNoBedPrice = resolveLottetourChildSlotPrice(d.childNoBedPrice, adultPrice, previous?.childNoBedPrice)
    let infantPrice = pickPreservedInfantPriceLottetour(d.infantPrice, previous?.infantPrice)
