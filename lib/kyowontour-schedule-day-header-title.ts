...
export type KyowontourHeaderTitleInput = {
  day: number
  /** LLM·추출 기존 title (참고만) */
  title: string
  /** 동선·관광 근거 텍스트 (읽기 전용) */
...
function extractPlacesInOrder(hay: string): string[] {
  ...
  for (let i = 0; i < h.length; i++) {
    for (const name of KYOWONTOUR_PLACE_NAMES) {
...
export function shouldReplaceKyowontourScheduleDayTitle(title: string, description: string): boolean {
 ...
 */
export function deriveKyowontourScheduleDayHeaderTitle(input: KyowontourHeaderTitleInput): string {
