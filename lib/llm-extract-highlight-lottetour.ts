/**
 * 롯데관광(lottetour) 전용 — Gemini로 상품 핵심 포인트 영역 추출·정리.
...
const LOTTE_SYSTEM = `당신은 롯데관광 상세 붙여넣기 원문에서 특정 구역만 분리하는 도우미입니다.
반드시 JSON 객체만 출력합니다. 마크다운·코드펜스·주석 금지.

롯데관광 규칙:
- 추출 대상은 「Point」「포인트」「상품포인트」「상품 포인트」 등으로 시작하고 ★(별표)·불릿과 함께 제시되는 상품 포인트 블록입니다.
- 별표/불릿으로 이어지는 핵심 문구만 포함하고, 일반 공지 전체를 넣지 마세요.
...
export async function extractHighlightFromLottetourLLM(rawText: string): Promise<{
  highlightPointsRaw: string | null
  highlightPoints: string | null
} | null> {
...
`
  ...
  const model = genAI.getGenerativeModel({
    model: getModelName(),
    systemInstruction: LOTTE_SYSTEM,
  })
  ...
  try {
    ...
    console.warn('[llm-extract-highlight-lottetour]', e instanceof Error ? e.message : e)
    return null
  }
}
