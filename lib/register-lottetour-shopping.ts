/**
 * 롯데관광: 옵션 블록·메타가 쇼핑 행으로 승격되지 않게 필터 + 쇼핑 횟수 정합.
...
export function parseLottetourShoppingPasteTab(raw: string): ShoppingStructured | null {
...
function parseLottetourShoppingVisitCount(hay: string): number | null {
...
export function lottetourShoppingRowLooksPlausible(r: ShoppingStructured['rows'][number]): boolean {
...
export function sanitizeLottetourShoppingStructured(
  ...
  if (paste) {
    const tabbed = parseLottetourShoppingPasteTab(paste)
    if (tabbed && tabbed.rows.length > 0) {
      const filtered = tabbed.rows.filter(lottetourShoppingRowLooksPlausible)
  ...
  const vc = parseLottetourShoppingVisitCount(shoppingSection)
  let shoppingCountText = structured.shoppingCountText ?? ''
  if (!shoppingCountText.trim() && vc != null && vc > 0) {
    shoppingCountText = `쇼핑 ${vc}회`
  }
  const filtered = structured.rows.filter(lottetourShoppingRowLooksPlausible)
  return {
    ...structured,
    rows: filtered,
...
export function finalizeLottetourRegisterParsedShopping(parsed: RegisterParsed): RegisterParsed {
  const st = parsed.detailBodyStructured?.shoppingStructured
  if (!st) return parsed
  const plausible = st.rows.filter(lottetourShoppingRowLooksPlausible)
  const n = parseLottetourShoppingVisitCount(st.shoppingCountText ?? '') ??
    parseLottetourShoppingVisitCount(parsed.shoppingSummaryText ?? '')
  const visitFromRows = (() => {
