...
import {
  parseLottetourTravelMainScheduleFlight,
...
export function parseFlightSectionLottetour(
  ...
  let blocks =
    full.length >= 32 ? parseLottetourTravelMainScheduleFlight(full) : null
  if (!blocks?.outbound?.flightNo || !blocks?.inbound?.flightNo) {
    blocks = full.length >= 32 ? parseLottetourTravelMainScheduleFlightFromTable(full) : null
  }
  if (!blocks?.outbound?.flightNo || !blocks?.inbound?.flightNo) {
    blocks = tryParseLottetourFlightBlocks(sectionClean)
  }
  const empty = createEmptyFlightLeg()
  const supplierBrandKey = 'lottetour'
  const expectFlightNumber = true

  if (!blocks?.outbound?.flightNo || !blocks?.inbound?.flightNo) {
    const reviewReasons = ['롯데관광 출발/도착 블록 또는 편명·일시 추출 실패']
    return {
  ...
  if (status !== 'success') reviewReasons.push('롯데관광 항공 구조화 일부만 성공 — 원문 검수')

  const selectedOutRaw = lottetourSynthesizePreferredRaw(blocks.outbound, '가는편')
